...
class ProposalController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $proposal_template = Proposal::with('proposalTemplates.individualLeadsUser', 'proposalTemplates.customer')->get();
            ...
            return DataTables::of($proposal_template)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $html .= '<a class="dropdown-item" href="'.route('crm.proposal_template.view', $row->proposalTemplates->id).'" id="view"><i class="far fa-eye text-primary"></i> View</a>';
                    ...
                    $html .= '<a class="dropdown-item" id="delete" href="'.route('crm.proposal_template.delete', $row->proposalTemplates->id).'"><i class="far fa-trash-alt text-primary"></i> Delete</a>';
                ...
                ->addColumn('name', function ($row) {
                    return strip_tags($row->proposalTemplates->customer->name ?? $row->proposalTemplates->individualLeadsUser->name);
                })
                ->addColumn('type', function ($row) {
                    return strip_tags(ucwords($row->proposalTemplates->related));
                })
                ->addColumn('subject', function ($row) {
                    return strip_tags(ucwords($row->proposalTemplates->subject));
                })
                ->editColumn('body', function ($row) {
                    return strip_tags($row->proposalTemplates->body);
                })
                ->rawColumns(['action', 'name', 'subject', 'type'])
                ->smart(true)
                ->make(true);
        }
        ...
        return view('crm::proposal.index', compact('customerLeads', 'individualLeadsUser', 'products', 'suppliers'));
    }
    ...
    public function store(ProposalTemplateAddRequest $request, FileUploaderServiceInterface $fileUploaderService, CodeGenerationServiceInterface $generator)
    {
        ...
        $proposal_template = new ProposalTemplate();
        ...
        $proposal = new \Modules\CRM\Entities\Proposal();
        ...
        if ($request->bcc != null) {
            array_push($emailArrayCCBcc, $request->bcc);
        }
        if ($request->cc != null) {
            array_push($emailArrayCCBcc, $request->cc);
        }
        $emailArray = array_merge($emailArrayCCBcc, $emailArray);
        ...
        return response()->json('Template created and send successfully');
    }
