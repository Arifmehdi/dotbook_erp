...
{
    public function smsServerSetup(Request $request)
    {
        if ($request->ajax()) {
            $server = SmsServer::all();

            return DataTables::of($server)
                ->addColumn('status', function ($row) {
                    $html = '';
                    if ($row['status'] == 1) {
                        $html .= '<div class="text-center"><a class="" href="'.route('communication.sms.server.active', [$row->id, 1]).'" id="status"><i class="fa-solid fa-check-to-slot"></i></a></div>';
                    } else {
                        $html .= '<div class="text-center"><a class="" href="'.route('communication.sms.server.active', [$row->id, 2]).'" id="status"><i class="fa-regular fa-check-to-slot"></i></a></div>';
                    }
                ...
                ->addColumn('edit', function ($row) {
                    $html = '';
                    $html .= '<div class="text-center"><a class="" href="'.route('communication.sms.serve.edit', $row->id).'" id="smsServerEdit"><i class="fa-solid fa-pen-to-square"></i></a></div>';

                    return $html;
                })
                ->addColumn('delete', function ($row) {
                    $html = '';
                    $html .= '<div class="text-center"><a class="" href="'.route('communication.sms.serve.delete', $row->id).'" id="delete"><i class="fa-solid fa-trash-can"></i></a></div>';
        ...
        return view('communication::sms.sms-server-setup');
    }

    public function smsServerStore(Request $request)
    {
        return $request->all();
        $request->validate([
