            ...
            <div class="row g-1">
                            ...
                            <div class="tab_contant receiver-phone">
                                ...
                                <div class="table-responsive-y">
                                    <form id="all_phone_checked_form" action="#" method="post">
                                        @csrf
                                        <table class="display data_tbl data__table phoneNumberBodyTable"
                                            id="phoneNumberBodyTable">
                                            <thead>
                                                <tr>
                ...
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form id="manual_sms_send" action="{{ route('communication.sms.manual-service.sms.send') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row g-2">
                                    {{-- sender selection part --}}
                                    <div class="col-sm-7">
                                        <select name="sms_serve_id" id="" class="form-control form-select">
                                    ...
                                    {{-- body selection part --}}
                                    <div class="col-sm-7">
                                        <select name="sms_format_body_id" id=""
                                    ...
                                    <div class="col-sm-7">
                                        <input type="text" name="sms_subject" id="sms_subject"
                                            class="form-control sms_subject_count" placeholder="{{ __('Sms Subject') }}">
...
@push('scripts')
    <script type="text/javascript">
        ...
        var phone_number_table = $('#phoneNumberBodyTable').DataTable({
            "processing": true,
            ...
            "ajax": {
                "url": "{{ route('communication.sms.manual-service') }}",
            },
            columns: [{
                    data: 'check',
                    name: 'check'
        ...
        $(document).on('click', '#tab_btn', function(e) {
            ...
            $('.loading_button').show();
            var url =
                "{{ route('communication.sms.manual-service-sms-status-wise-list', ['statusType' => ':statusType']) }}";
            url = url.replace(':statusType', statusType);

            phone_number_table = $('#phoneNumberBodyTable').DataTable({
                "processing": false,
                "serverSide": true,
                "ajax": {
                    "url": url,
        ...
        $('.mailFilter').on('change', function() {
            ...
            $('.loading_button').show();
            var url =
                "{{ route('communication.sms.manual-service-sms-list', ['filterType' => ':filterType', 'filterKey' => ':filterKey']) }}";
        ...
        jQuery(document).on('click', '.active_inactive_status', function(ev) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    toastr.success(data);
                    //  phone_number_table.destroy();
                    $('.phoneNumberBodyTable').DataTable().ajax.reload();
                },
                error: function(err) {
                    $('.loading_button').hide();
                    $('.error').html('');
        ...
        $('#import_phone_number_modal').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('communication.sms.manual-service.sms-import-modal') }}", function(data) {
        ...
        $('#manual_sms_send').on('submit', function(e) {
            e.preventDefault();
