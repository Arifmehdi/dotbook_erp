            ...
            <form id="add_cnf_agent_form" action="{{ route('lc.cnf.agents.store') }}">
                ...
                <div class="form-group row mt-1">
                    <div class="col-md-3">
                        ...
                        <input type="text" name="name" class="form-control cnf_add_input" data-name="Agent name"
                            id="name" placeholder="Agent Company Name" />
                        <span class="error error_cnf_name"></span>
                    </div>

                    <div class="col-md-3">
                        <label><b>@lang('menu.phone') </b> <span class="text-danger">*</span></label>
                        <input type="text" name="phone" class="form-control cnf_add_input" data-name="Phone number"
                            id="phone" placeholder="Phone number" />
                        <span class="error error_cnf_phone"></span>
                    </div>
    ...
    $('#add_cnf_agent_form').on('submit', function(e) {
        ...
        $.each(inputs, function(key, val) {
            ...
            if (idValue == '') {
                ...
                $('.error_cnf_' + inputId).html(fieldName + ' is required.');
            }
        });
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('.loading_button').hide();
                $('#cnf_agent_id').append('<option value="' + data.id + '">' + data.name +
                    '</option>');
                $('#cnf_agent_id').val(data.id);
