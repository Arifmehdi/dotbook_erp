...
<div class="modal-header">
    <ul class="breadcrumb wizard" style="width: 100%; margin-bottom: 0px !important;">
        @php
            $jobApplyStatus = $alreadyMailedInterviewer['status'];
        @endphp
    ...
    <div class="">
                ...
                <div class="card">
                    <div class="card-body text-center">
                        ...
                        <div class="profile-top p-0">
                            @if ($alreadyMailedInterviewer['photo'] != null)
                                <img class="img-fluid rounded-circle" id="photo"
                                    src="{{ asset('website/') }}/{{ $alreadyMailedInterviewer['photo'] }}"
                                    width="175" height="180" alt="Image not found">
                            @else
                                <div class="part-img">
                        ...
                        <h6 class="font-weight-semibold mb-0 mt-3">
                            {{ $alreadyMailedInterviewer->job_applied['job_title'] ?? 'no available' }}</h6>
                        <h6 class="font-weight-semibold mb-0">
                            {{ $alreadyMailedInterviewer['first_name'] ?? 'no available' }}
                            {{ $alreadyMailedInterviewer['last_name'] ?? 'no available' }}</h6>
                        <h6 class="font-weight-semibold mb-0"></h6>
                        <span class="d-block mb-3"><b>{{ __('Phone') }}:
                                {{ $alreadyMailedInterviewer['mobile'] ?? 'no available' }} </b></span>
                    </div>
                </div>
            </div>
            <div class="col-lg-8">
                <div class="row g-3">
                        ...
                        <div>
                            <table class="table table-borderless">
                                <tr>
                                    ...
                                    <td>: {{ $alreadyMailedInterviewer['first_name'] ?? 'no available' }}
                                        {{ $alreadyMailedInterviewer['last_name'] ?? 'no available' }}</td>
                                    <th class="bg-transparent text-dark">@lang('menu.phone') </th>
                                    <td>: {{ $alreadyMailedInterviewer['mobile'] ?? 'no available' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-transparent text-dark">@lang('menu.email') </th>
                                    <td>: {{ $alreadyMailedInterviewer['email'] ?? 'no available' }}</td>
                                    <th class="bg-transparent text-dark">{{ __('Source') }} </th>
                                    <td>: {{ $alreadyMailedInterviewer['sourch'] ?? 'no available' }} </td>
                                </tr>
                                <tr>
                                    <th class="bg-transparent text-dark">@lang('menu.city') </th>
                                    <td>: {{ $alreadyMailedInterviewer['city'] ?? 'no available' }}</td>
                                    <th class="bg-transparent text-dark">@lang('menu.country') </th>
                                    <td>: {{ $alreadyMailedInterviewer['country'] ?? 'no available' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-transparent text-dark">{{ __('Website') }} </th>
                                    <td>: {{ $alreadyMailedInterviewer['website_url'] ?? 'no available' }}</td>
                                    <th class="bg-transparent text-dark">{{ __('LinkedIn') }} </th>
                                    <td>: {{ $alreadyMailedInterviewer['linkedin_url'] ?? 'no available' }}</td>
                                </tr>
                            </table>
                        </div>
                    </div>
                    <div class="col-12 border rounded">
                        ...
                        <table class="table  table-borderless">
                            ...
                            <tbody>
                                @foreach (json_decode($alreadyMailedInterviewer['education'], true) as $education)
                                    <tr>
                    ...
                    <div class="col-12 border rounded">
                        ...
                        <table class="table  table-borderless">
                            ...
                            <tbody>
                                @foreach (json_decode($alreadyMailedInterviewer['experience'], true) as $experience)
                                    <tr>
                ...
                <div class="card">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                            @php
                                $skills = explode(' ', $alreadyMailedInterviewer['skill']);
                            @endphp
                            <tbody>
                ...
                <div class="card">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                            <tbody>
                                <tr>
                                    <td>{{ $alreadyMailedInterviewer['location'] ?? 'no available' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="row">
            <div class="col-12 border rounded">
                ...
                <table class="table  table-borderless">
                    @if (isset($alreadyMailedInterviewer['cover_letter']))
                        <p class="py-3">
                            @php
                                $cover_letter = $alreadyMailedInterviewer['cover_letter'];
                                $extension = pathinfo($cover_letter, PATHINFO_EXTENSION);
                            @endphp
                            @if ($extension == 'pdf')
                                <iframe src="{{ asset('/website/' . $alreadyMailedInterviewer['cover_letter']) }}"
                                    width="100%" height="600"></iframe>
                            @else
                                <img src="{{ asset('/website/' . $alreadyMailedInterviewer['cover_letter']) }}"
        ...
        <div class="row">
            <div class="col-12 border rounded">
                ...
                <table class="table  table-borderless">
                    @if (isset($alreadyMailedInterviewer['resume']))
                        <p class="py-3">
                            @php
                                $resume = $alreadyMailedInterviewer['resume'];
                                $extension = pathinfo($resume, PATHINFO_EXTENSION);
                            @endphp
                            @if ($extension == 'pdf')
                                <iframe src="{{ asset('/website/' . $alreadyMailedInterviewer['resume']) }}"
                                    width="100%" height="600"></iframe>
                            @else
                                <img src="{{ asset('/website/' . $alreadyMailedInterviewer['resume']) }}"
                                    height="200" width="100%" />
                            @endif
                        </p>
                    @endif
            ...
            <div class="modal-footer p-3 d-flex justify-content-between">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <form
                        action="{{ route('hrm.applicant_move_to_participate_interview_single', ['id' => $alreadyMailedInterviewer['id']]) }}"
                        ...
                        <select name="status" class="form-controll me-2" style="padding: 0px 6px;"
                            ...
                            <option value="{{ JobAppliedStatus::InterviewParticipant->value }}">
                                {{ __('Interview Participated') }}</option>
