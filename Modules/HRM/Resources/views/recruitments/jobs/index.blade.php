...
<div class="body-wraper">
            @if(auth()->user()->can('web_manage_job') || auth()->user()->can('web_add_job')
            || auth()->user()->can('web_edit_job') || auth()->user()->can('web_delete_job'))
                <div class="main__content">
                    <div class="sec-name">
                        <div class="name-head">
                            <h6>@lang('menu.jobs')</h6>
                        </div>
                        <div class="d-flex">
                            <div id="exportButtonsContainer">
                                @if(auth()->user()->can('web_add_job'))
                                <a href="#" class="btn text-white btn-sm" id="add_customer"><span><i class="fa-thin fa-circle-plus fa-2x"></i><br>@lang('menu.new_job')</span></a>
                                @endif
                            </div>
                ...
                @if(auth()->user()->can('web_manage_job') || auth()->user()->can('web_edit_job') || auth()->user()->can('web_delete_job'))
                <div class="p-15">
                                    ...
                                    <table class="display data_tbl data__table customerTable">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-start">@lang('menu.job_type')</th>
                                                <th class="text-start">@lang('menu.job_title')</th>
                                                <th class="text-start">@lang('menu.category')</th>
                                                <th class="text-start">@lang('menu.location')</th>
                                                <th class="text-start">@lang('menu.deadline')</th>
...
<script>
    $(document).ready(function() {
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.jobs.create') }}", function(data) {
