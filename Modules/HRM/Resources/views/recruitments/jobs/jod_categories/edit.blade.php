...
<div class="modal-dialog col-60-modal" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">@lang('menu.edit_job_category') <span class="type_name"></span></h6>
            <a href="#" class="close-btn" data-bs-dismiss="modal" aria-label="Close"><span
                    class="fas fa-times"></span></a>
        </div>
        <div class="modal-body">
            <form id="submit_customer_basic_form"
                action="{{ route('website.job-categories.update', $job_categories->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="form-group row p-1">
                        ...
                        <div class="col-sm-9">
                            <input required type="text" id="name" name="name" class="form-control"
                                value="{{ $job_categories->name }}" placeholder="Name">
                        </div>
                    </div>
                    <div class="form-group row p-1">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control form-select" name="status">
                                ...
                                <option @if ($job_categories->status == 1) selected @endif value="1">Active</option>
                                <option @if ($job_categories->status == 0) selected @endif value="0">In-Active
                                </option>
                            </select>
                        </div>
                    </div>
