...
<div class="body-wraper">
            @if(auth()->user()->can('web_manage_job_category') || auth()->user()->can('web_add_job_category')
            || auth()->user()->can('web_edit_job_category') || auth()->user()->can('web_delete_job_category'))
                <div class="main__content">
                    <div class="sec-name">
                        <div class="name-head">
                            <h6>@lang('menu.job_category')</h6>
                        </div>
                        <div class="d-flex">
                            <div id="exportButtonsContainer">
                                @if(auth()->user()->can('web_add_job_category'))
                                <a href="#" class="btn text-white btn-sm" id="add_customer"><span><i class="fa-thin fa-circle-plus fa-2x"></i><br>@lang('menu.new_job_category')</span></a>
                                @endif
                            </div>
                ...
                @if(auth()->user()->can('web_manage_job_category') || auth()->user()->can('web_edit_job_category') || auth()->user()->can('web_delete_job_category'))
                <div class="p-15">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.job-categories.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.job-categories.create') }}", function(data) {
