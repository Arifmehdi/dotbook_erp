<!-- Edit Modal -->
<form id="update_form" action="{{ route('hrm.salaryAdjustments.update', $salaryAdjustment->id) }}" method="POST"
    enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="id" value="{{ $salaryAdjustment->id }}" />

    <div class="col-xl-12 col-md-12">
        <label><b> {{ __('Employee') }}</b> <span class="text-danger">*</span></label>
        <select name="employee_id" required class="form-control submit_able form-select" id="employee_id" autofocus="">
            <option value="">Select</option>
            @foreach ($employees as $employee)
                <option value="{{ $employee->id }}" @if ($salaryAdjustment->employee_id == $employee->id) {{ 'selected' }} @endif>
    ...
    <div class="row">
        <div class="form-group col-xl-6 col-md-6">
            ...
            <input type="number" name="amount" value="{{ $salaryAdjustment->amount }}"
        ...
        <div class="col-xl-6 col-md-6">
            ...
            <select name="type" class="form-control submit_able form-select" id="type">
                ...
                <option value="1" @if ($salaryAdjustment->type == 1) {{ 'selected' }} @endif>Addition</option>
                <option value="2" @if ($salaryAdjustment->type == 2) {{ 'selected' }} @endif>Deduction</option>
            ...
            <select name="month" class="form-control form-control-sm add_input" id="month"
                ...
                @foreach ($months as $month => $monthName)
                    <option value="{{ $month }}"
                        ...
                        @if ($month == $salaryAdjustment->month) {{ 'selected' }} @endif> {{ $monthName }}</option>
                @endforeach
            </select>
            ...
            <select name="year" class="form-control form-control-sm add_input" id="year"
                ...
                @for ($year = 2020; $year <= date('Y') + 5; $year++)
                    <option value="{{ $year }}" @php $today = carbon\Carbon::now(); $today->year; @endphp
                        @if ($salaryAdjustment->year == $year) {{ 'selected' }} @endif>{{ $year }}</option>
                @endfor
            </select>
    ...
    <div class="row">
        <div class="form-group col-xl-12 col-md-12">
            ...
            <textarea cols="30" rows="3" name="description" class="form-control form-control-sm add_input ckEditor"
                id="description" placeholder="{{ __('Description') }}"> {{ $salaryAdjustment->description }}</textarea>
