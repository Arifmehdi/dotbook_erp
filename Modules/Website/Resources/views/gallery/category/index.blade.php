...
<div class="body-wraper">
    @if(auth()->user()->can('web_manage_gallery_category') || auth()->user()->can('web_add_gallery_category')
    || auth()->user()->can('web_edit_gallery_category') || auth()->user()->can('web_delete_gallery_category'))
    <div class="main__content">
        <div class="sec-name">
            <div class="name-head">
                <h6>@lang('menu.gallery_category')</h6>
            </div>
            <x-all-buttons>
                <x-add-button :can="'web_add_gallery_category'" id="add_customer" />
                <x-slot name="after">
                    <x-help-button />
                </x-slot>
            </x-all-buttons>
    ...
    @if(auth()->user()->can('web_manage_gallery_category') || auth()->user()->can('web_edit_gallery_category') || auth()->user()->can('web_delete_gallery_category'))
    <div class="p-15">
                        ...
                        <table class="display data_tbl data__table customerTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-start">@lang('menu.slug')</th>
                                    <th class="text-start">@lang('menu.icon')</th>
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.gallery-categories.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.gallery-categories.create') }}", function(data) {
