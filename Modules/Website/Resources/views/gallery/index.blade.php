...
<div class="body-wraper">
    @if(auth()->user()->can('web_manage_gallery') || auth()->user()->can('web_add_gallery')
    || auth()->user()->can('web_edit_gallery') || auth()->user()->can('web_delete_gallery'))
    <div class="main__content">
        <div class="sec-name">
            <div class="name-head">
                <h6>@lang('menu.gallerys')</h6>
            </div>
            <x-all-buttons>
                <x-add-button :can="'web_add_gallery'" id="add_customer" />
                <x-slot name="after">
                    <x-help-button />
                </x-slot>
            </x-all-buttons>
    ...
    @if(auth()->user()->can('web_manage_gallery') || auth()->user()->can('web_edit_gallery') || auth()->user()->can('web_delete_gallery'))
    <div class="p-15">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.gallery.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.gallery.create') }}", function(data) {
