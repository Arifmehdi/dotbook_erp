...
<div class="body-wraper">
    @if (auth()->user()->can('web_manage_partner') || auth()->user()->can('web_add_partner')
    || auth()->user()->can('web_edit_partner')|| auth()->user()->can('web_delete_partner'))
    <div class="main__content">
        <div class="sec-name">
            <div class="name-head">
                <h6>@lang('menu.partners')</h6>
            </div>
            <x-all-buttons>
                <x-add-button id="add_customer" :can="'web_add_partner'" />
                <x-slot name="after">
                    <x-help-button />
                </x-slot>
            </x-all-buttons>
    ...
    @if (auth()->user()->can('web_manage_partner') || auth()->user()->can('web_edit_partner')|| auth()->user()->can('web_delete_partner'))
    <div class="p-15">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.partners.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.partners.create') }}", function(data) {
