...
<div class="body-wraper">
    @if(auth()->user()->can('web_add_client') || auth()->user()->can('web_manage_client') || auth()->user()->can('web_edit_client') || auth()->user()->can('web_delete_client'))
    <div class="main__content">
        <div class="sec-name">
            <div class="name-head">
                <h6>@lang('menu.clients')</h6>
            </div>
            <div class="d-flex gap-2">
                <x-table-stat :items="[
                    ['id' => 'total_customer', 'name' => __('Total Clients'), 'value' => $total_clients]
                ]" />
                <x-all-buttons>
                    <x-add-button id="add_customer" :can="'web_add_client'" />
                    <x-slot name="after">
                        <x-help-button />
                    </x-slot>
                </x-all-buttons>
    ...
    @if(auth()->user()->can('web_manage_client') || auth()->user()->can('web_edit_client') || auth()->user()->can('web_delete_client'))
    <div class="p-15">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.clients.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.client.create.basic.modal') }}", function(data) {
