...
<div class="body-wraper">
    @if(auth()->user()->can('web_manage_award') || auth()->user()->can('web_add_award')
    || auth()->user()->can('web_edit_award') || auth()->user()->can('web_delete_award'))
    <div class="main__content">
        <div class="sec-name">
            <div class="name-head">
                <h6>@lang('menu.awards')</h6>
            </div>
            <div class="d-flex">
                <div id="exportButtonsContainer">
                    @if(auth()->user()->can('web_add_award'))
                    <a href="#" class="btn text-white btn-sm" id="add_customer"><span><i class="fa-thin fa-circle-plus fa-2x"></i><br>@lang('menu.add_new')</span></a>
                    @endif
                </div>
    ...
    @if(auth()->user()->can('web_manage_award') || auth()->user()->can('web_edit_award') || auth()->user()->can('web_delete_award'))
    <div class="p-15">
                        ...
                        <table class="display data_tbl data__table customerTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-start">@lang('menu.link')</th>
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.award.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.award.create') }}", function(data) {
