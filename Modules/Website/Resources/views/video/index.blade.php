...
<div class="body-wraper">
    @if(auth()->user()->can('web_manage_video') || auth()->user()->can('web_add_video')
    || auth()->user()->can('web_edit_video') || auth()->user()->can('web_delete_video'))
    <div class="main__content">
        <div class="sec-name">
            <div class="name-head">
                <h6>@lang('menu.videos')</h6>
            </div>
            <x-all-buttons>
                <x-add-button :can="'web_add_video'" id="add_customer" />
                <x-slot name="after">
                    <x-help-button />
                </x-slot>
            </x-all-buttons>
    ...
    @if(auth()->user()->can('web_manage_video') || auth()->user()->can('web_edit_video') || auth()->user()->can('web_delete_video'))
    <div class="p-15">
        <div class="card">
            <div class="card-body">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.video.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.video.create') }}", function(data) {
