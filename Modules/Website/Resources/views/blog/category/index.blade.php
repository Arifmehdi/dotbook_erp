...
<div class="body-wraper">
    @if(auth()->user()->can('web_manage_blog_category') || auth()->user()->can('web_add_blog_category')
    || auth()->user()->can('web_edit_blog_category') || auth()->user()->can('web_delete_blog_category'))
    <div class="main__content">
        <div class="sec-name">
            <div class="name-head">
                <h6>@lang('menu.blog_category')</h6>
            </div>
            <x-all-buttons>
                <x-add-button :can="'web_add_blog_category'" id="add_customer" />
                <x-slot name="after">
                    <x-help-button />
                </x-slot>
            </x-all-buttons>
    ...
    @if(auth()->user()->can('web_manage_blog_category') || auth()->user()->can('web_edit_blog_category') || auth()->user()->can('web_delete_blog_category'))
    <div class="p-15">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.blog-categories.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.blog-categories.create') }}", function(data) {
