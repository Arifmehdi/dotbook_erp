...
<div class="body-wraper">
    @if(auth()->user()->can('web_manage_page') || auth()->user()->can('web_add_page')
    || auth()->user()->can('web_edit_page') || auth()->user()->can('web_delete_page'))
    <div class="main__content">
        <div class="sec-name">
            ...
            <x-all-buttons>
                <x-add-button :can="'web_add_page'" id="add_customer" />
                <x-slot name="after">
                    <x-help-button />
                </x-slot>
            </x-all-buttons>
    ...
    @if(auth()->user()->can('web_manage_page') || auth()->user()->can('web_edit_page') || auth()->user()->can('web_delete_page'))
    <div class="p-15">
                        ...
                        <table class="display data_tbl data__table customerTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-start">@lang('menu.url')</th>
                                    <th class="text-start">@lang('menu.position')</th>
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.pages.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.pages.create') }}", function(data) {
