...
@section('content')
    <div class="body-wraper">
        @if (auth()->user()->can('web_manage_testimonial') ||
                auth()->user()->can('web_add_testimonial') ||
                auth()->user()->can('web_edit_testimonial') ||
                auth()->user()->can('web_delete_testimonial'))
            <div class="main__content">
                <div class="sec-name">
                    <div class="name-head">
                        <h6>@lang('menu.testimonial')</h6>
                    </div>
                    <x-all-buttons>
                        <x-add-button :can="'web_add_testimonial'" id="add_customer" />
                        <x-slot name="after">
                            <x-help-button />
                        </x-slot>
                    </x-all-buttons>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('.customerTable').DataTable({
                ...
                ajax: "{{ route('website.testimonial.index') }}",
                columns: [{
                    data: 'action',
                    name: 'action'
                }, {
            ...
            $('#add_customer').on('click', function(e) {
                e.preventDefault();
                $.get("{{ route('website.testimonial.create') }}", function(data) {
