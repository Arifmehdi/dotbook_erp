...
<div class="body-wraper">
    @if(auth()->user()->can('web_manage_category') || auth()->user()->can('web_add_category')
    || auth()->user()->can('web_edit_category') || auth()->user()->can('web_delete_category'))
    <div class="main__content">
        <div class="sec-name">
            <div class="name-head">
                <h6>@lang('menu.categories')</h6>
            </div>
            <x-all-buttons>
                <x-add-button id="add_customer" :can="'web_add_category'" />
                <x-slot name="after">
                    <x-help-button />
                </x-slot>
            </x-all-buttons>
    ...
    @if(auth()->user()->can('web_manage_category') || auth()->user()->can('web_edit_category')
    || auth()->user()->can('web_delete_category'))
    <div class="p-15">
...
<script>
    $(document).ready(function() {
        ...
        var table = $('.customerTable').DataTable({
            ...
            , ajax: "{{ route('website.categories.index') }}"
            , columns: [{
                    data: 'action'
                    , name: 'action'
                }
        ...
        $('#add_customer').on('click', function(e) {
            e.preventDefault();
            $.get("{{ route('website.categories.create') }}", function(data) {
