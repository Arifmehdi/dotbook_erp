...
class DayBookUtil
{
    ...
    public function voucherType($voucherTypeId)
    {
        $data = [
            ...
            7 => ['name' => 'Expenses', 'id' => 'expense_id', 'voucher_no' => 'expense_voucher', 'details_id' => 'expense_id', 'link' => 'vouchers.expenses.show'],
            8 => ['name' => 'Stock Adjustment', 'id' => 'stock_adjustment_id', 'voucher_no' => 'stock_adjustment_voucher', 'details_id' => 'stock_adjustment_id', 'link' => 'stock.adjustments.show'],
            9 => ['name' => 'Receipt', 'id' => 'payment_id', 'voucher_no' => 'payment_voucher', 'details_id' => 'payment_id', 'link' => 'vouchers.receipts.show'],
            10 => ['name' => 'Payment', 'id' => 'payment_id', 'voucher_no' => 'payment_voucher', 'details_id' => 'payment_id', 'link' => 'vouchers.payments.show'],
            11 => ['name' => 'Contra', 'id' => 'contra_id', 'voucher_no' => 'contra_voucher', 'details_id' => 'contra_id', 'link' => 'vouchers.contras.show'],
            12 => ['name' => 'Journal', 'id' => 'journal_id', 'voucher_no' => 'journal_voucher', 'details_id' => 'journal_id', 'link' => 'vouchers.journals.show'],
            13 => ['name' => 'Daily Stock', 'id' => 'daily_stock_id', 'voucher_no' => 'daily_stock_voucher', 'details_id' => 'daily_stock_id', 'link' => 'daily.stock.show'],
            14 => ['name' => 'Receive Stock', 'id' => 'receive_stock_id', 'voucher_no' => 'receive_stock_voucher', 'details_id' => 'receive_stock_id', 'link' => 'purchases.receive.stocks.show'],
            15 => ['name' => 'Stock Issue', 'id' => 'stock_issue_id', 'voucher_no' => 'stock_issue_voucher', 'details_id' => 'stock_issue_id', 'link' => 'stock.issue.show'],
        ];
        ...
        if ($request->voucher_details == 1) {
            ...
            foreach ($daybook?->payment?->descriptions as $description) {
                ...
                if ($showingAccountId != $description->account_id) {
                    ...
                    $amount_type = $description->amount_type == 'dr' ? ' <strong>Dr.</strong>' : ' <strong>Cr.</strong>';
                    $__amount = ' : ' . $amount . $amount_type;
                    $assignedUser = $description?->user ? (' - <strong>' . __('Sr.') . '</strong> ' . $description?->user?->prefix . ' ' . $description?->user?->name . ' ' . $description?->user?->last_name) : '';
                    ...
                    if (count($description->references) > 0) {
                        ...
                        $referencesDetails .= '<tr><td colspan="2" style="line-height:0.9 !important;"><strong>(Against References) :</strong>';
    ...
    public function DailyStockDetails($request, $daybook)
    {
        ...
        if ($request->voucher_details == 1) {
            ...
            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:1 !important;" class="w-60"><strong>' . __('menu.total_qty') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:1 !important;"> : ' . \App\Utils\Converter::format_in_bdt($daybook?->dailyStock?->total_qty) . '</td>';
            $voucherDetails .= '</tr>';

            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:1 !important;" class="w-60"><strong>' . __('menu.total_stock_value') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:1 !important;"> : ' . \App\Utils\Converter::format_in_bdt($daybook?->dailyStock?->total_stock_value) . '</td>';
            $voucherDetails .= '</tr>';
            $voucherDetails .= '</table>';
        }
        ...
        if ($request->inventory_list == 1) {
            ...
            foreach ($daybook->dailyStock->dailyStockProducts as $dailyStockProduct) {
                ...
                $inventoryDetails .= '<tr>';
                $inventoryDetails .= '<td style="line-height:1 !important;" class="w-50">- ' . $dailyStockProduct?->product?->name . $variantName . '</td>';

                $inventoryDetails .= '<td style="line-height:1 !important;">' . \App\Utils\Converter::format_in_bdt($dailyStockProduct->quantity) . '/' . $dailyStockProduct?->unit . '</td>';

                $inventoryDetails .= '<td style="line-height:1 !important;">(' . \App\Utils\Converter::format_in_bdt($dailyStockProduct->quantity) . 'X' . $dailyStockProduct->unit_cost_inc_tax . ')</td>';

                $inventoryDetails .= '<td style="line-height:1 !important;">=' . \App\Utils\Converter::format_in_bdt($dailyStockProduct->subtotal) . '</td>';
                $inventoryDetails .= '</tr>';
            }
            $inventoryDetails .= '</table>';
        }
    ...
    public function receiveStockDetails($request, $daybook)
    {
        ...
        if ($request->voucher_details == 1) {
            ...
            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:1 !important;" class="w-60"><strong>' . __('menu.challan_no') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:1 !important;"> : ' . $daybook?->receiveStock?->challan_no . '</td>';
            $voucherDetails .= '</tr>';

            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:1 !important;" class="w-60"><strong>' . __('menu.challan_date') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:1 !important;"> : ' . $daybook?->receiveStock?->challan_date . '</td>';
            $voucherDetails .= '</tr>';

            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:1 !important;" class="w-60"><strong>' . __('menu.total_item') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:1 !important;"> : ' . \App\Utils\Converter::format_in_bdt($daybook?->receiveStock?->total_item) . '</td>';
            $voucherDetails .= '</tr>';

            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:1 !important;" class="w-60"><strong>' . __('menu.total_qty') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:1 !important;"> : ' . \App\Utils\Converter::format_in_bdt($daybook?->receiveStock?->total_qty) . '</td>';
            $voucherDetails .= '</tr>';
        ...
        if ($request->inventory_list == 1) {
            ...
            foreach ($daybook->receiveStock->receiveStockProducts as $receiveStockProduct) {
                ...
                $inventoryDetails .= '<tr>';
                $inventoryDetails .= '<td style="line-height:1 !important;" class="w-50">- ' . $receiveStockProduct?->product?->name . $variantName . '</td>';
                ...
                $inventoryDetails .= '<td style="line-height:1 !important;">' . \App\Utils\Converter::format_in_bdt($receivedQty) . '/' . $receiveStockProduct?->receiveUnit?->code_name . '</td>';
    ...
    public function StockIssueDetails($request, $daybook)
    {
        ...
        if ($request->voucher_details == 1) {
            ...
            $voucherDetails .= '<tr>';
            $voucherDetails .= '<td style="line-height:1 !important;" class="w-60"><strong>' . __('menu.total_stock_value') . '</strong></td>';
            $voucherDetails .= '<td style="line-height:1 !important;"> : ' . \App\Utils\Converter::format_in_bdt($daybook?->stockIssue?->net_total_value) . '</td>';
            $voucherDetails .= '</tr>';
            $voucherDetails .= '</table>';
        }
        ...
        if ($request->inventory_list == 1) {
            ...
            foreach ($daybook->stockIssue->issueProducts as $issueProduct) {
                ...
                $inventoryDetails .= '<tr>';
                $inventoryDetails .= '<td style="line-height:1 !important;" class="w-50">- ' . $issueProduct?->product?->name . $variantName . '</td>';
                ...
                $inventoryDetails .= '<td style="line-height:1 !important;">' . \App\Utils\Converter::format_in_bdt($issuedQty) . '/' . $issueProduct?->issueUnit->code_name . '</td>';

                $inventoryDetails .= '<td style="line-height:1 !important;">(' . \App\Utils\Converter::format_in_bdt($issuedQty) . 'X' . $unitCostIncTax . ')</td>';

                $inventoryDetails .= '<td style="line-height:1 !important;">=' . \App\Utils\Converter::format_in_bdt($issueProduct->subtotal) . '</td>';
                $inventoryDetails .= '</tr>';
            }
            $inventoryDetails .= '</table>';
        }
