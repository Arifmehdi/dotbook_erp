...
class Converter
{
     ...
     */
    public static function format_in_bdt($number = 0, $decimal = 2)
    {
        if (isset($number)) {
            // return \number_format($number, 2, '.', ',');
            $fmt = new NumberFormatter('bn_BDT', NumberFormatter::DECIMAL);
            $fmt->setAttribute(NumberFormatter::MIN_FRACTION_DIGITS, $decimal);
            $fmt->setAttribute(NumberFormatter::MAX_FRACTION_DIGITS, $decimal);
            $fmt->setAttribute(NumberFormatter::DECIMAL_ALWAYS_SHOWN, $decimal);

            return self::bn2en($fmt->format(floatval($number)));
        }
    }
    ...
    public static function format_in_text($number)
    {
        ...
        // return \ucwords($fmt->format(floatval($number))); // Ten Million Two Thousand Three Hundred Forty-five Point Eight Nine
        return \ucfirst($fmt->format(floatval($number))); // Ten million two thousand three hundred forty-five point eight nine
    }
    ...
    public static function format_in_bdt_bn($number)
    {
        ...
        return self::en2bn($fmt->format(floatval($number)));
    }
}
