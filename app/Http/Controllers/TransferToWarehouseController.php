...
class TransferToWarehouseController extends Controller
{
    ...
    {
        $transfer = TransferStockToWarehouse::with('warehouse', 'Transfer_products', 'Transfer_products.product', 'Transfer_products.variant')
            ->where('id', $id)
            ->first();
    ...
    {
        $transfer = TransferStockToWarehouse::with('warehouse', 'Transfer_products', 'Transfer_products.product', 'Transfer_products.variant')->where('id', $transferId)->first();
        $qty_limits = [];

        foreach ($transfer->Transfer_products as $transfer_product) {

            $productBranch = ProductBranch::where('product_id', $transfer_product->product_id)->first();
    ...
    public function productSearch($product_code)
    {
        ...
        $product = Product::with(['variants', 'tax', 'unit'])
            ...
            if ($productBranch) {
                ...
                if ($product->type == 2) {
                    ...
                    if ($productBranch->product_quantity > 0) {
                        ...
                        return response()->json(['errorMsg' => 'Stock is out of this item in this Company']);
                    }
                }
            } else {

                return response()->json(['errorMsg' => 'This item is not available in this Company.']);
            }
        } else {
            ...
            if ($variant_product) {
                ...
                if ($variant_product) {
                    ...
                    if (is_null($productBranch)) {
                        ...
                        return response()->json(['errorMsg' => 'This item is not available in this shop']);
                    }
                    ...
                    if ($productBranch && $productBranchVariant) {
                        ...
                        return response()->json(['errorMsg' => 'This item is not available in this branch.']);
                    }
                }
            }
        }
    ...
    public function checkBranchSingleProduct($product_id)
    {
        ...
        if ($productBranch) {
            ...
            if ($productBranch->product_quantity > 0) {
                ...
                return response()->json(['errorMsg' => 'Stock is out of this item(variant) of this Company']);
            }
        } else {

            return response()->json(['errorMsg' => 'This item is not available in this ompany.']);
        }
    }
