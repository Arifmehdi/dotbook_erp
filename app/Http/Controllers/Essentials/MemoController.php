...
class MemoController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($memos)
                ->addColumn('action', function ($row) {
                    ...
                    if ($row->is_author == 1) {
                        $html .= '<a href="' . route('memos.edit', [$row->id]) . '" class="action-btn edit-btn" title="Edit"><span class="fas fa-edit"></span></a>';
                ...
                ->rawColumns(['action', 'created_at', 'heading', 'description'])
                ->make(true);
        }
    ...
    {
        $addons = DB::table('addons')->select('todo')->first();
        abort_if($addons->todo == 0, 403, 'Access Forbidden.');
        $memo = Memo::where('id', $id)->first(['id', 'heading', 'description']);
        return view('essentials.memos.ajax_view.edit', compact('memo'));
    }
