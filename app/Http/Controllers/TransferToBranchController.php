...
class TransferToBranchController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $transfers = DB::table('transfer_stock_to_branches')
                ->leftJoin('warehouses', 'transfer_stock_to_branches.warehouse_id', 'warehouses.id')->select(
    ...
    public function edit($transferId)
    {
        ...
        $transfer = DB::table('transfer_stock_to_branches')->where('id', $transferId)->select('id', 'warehouse_id', 'date')->first();
    ...
    public function delete($transferId)
    {
        ...
        if (! is_null($deleteTransferToBranch)) {
            ...
            foreach ($storedTransferredProducts as $transfer_product) {
                ...
                $this->productStockUtil->adjustBranchStock($transfer_product->product_id, $transfer_product->product_variant_id);
            }
        }
            ...
            if ($variant_product) {
                ...
                if ($productWarehouse && $productWarehouseVariant) {
                    ...
                    if ($productWarehouseVariant->variant_quantity > 0) {
                        ...
                        return response()->json(['errorMsg' => 'Stock is out of this item(variant) of this warehouse']);
                    }
                } else {
