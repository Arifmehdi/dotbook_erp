<table class="w-100">
    <thead>
        <tr>
            <th class="header_text ps-1">@lang('menu.liabilities')</th>
            <th class="header_text ps-1" style="border-left: 1px solid black;">@lang('menu.assets')</th>
        </tr>
    </thead>
    ...
    <tbody>
        <tr>
                                ...
                                @if ($formatOfReport == 'detailed')
                                    <table class="group_account_table ms-2">
                                        ...
                                        @foreach ($fixedAssetsAccount->accounts as $account)
                                            @if ($account->closing_balance)
                                                <tr>
                                                    <td class="group_account_name ps-1"><a href="{{ route('accounting.accounts.ledger', [$account->account_id, 'accountId', $fromDate, $toDate]) }}">{{ $account->account_name }} (@lang('menu.ledger'))</a></td>
                                                    <td class="group_account_balance text-end">
                                                        {{ $account->closing_balance_side == 'dr' ? '(-)' : '' }}
                                                        {{ \App\Utils\Converter::format_in_bdt($account->closing_balance) }}
                                                    </td>
            ...
            <td class="debit_area" style="border-left: 1px solid #000;">
