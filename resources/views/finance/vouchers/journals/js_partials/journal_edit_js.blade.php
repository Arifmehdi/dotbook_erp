<script>
    ...
    // var myArray = [];
    var myArray = @json($costCentreArr);
    var rowIndex = {{ $totalEntries }};
    ...
    $(document).on('input', '#search_account', function(e) {
        ...
        delay(function() { searchAccount(__keyword, onlyType); }, 200);
    });
    ...
    var global_default_main_group_number = '';
    ...
    $(document).on('click', '.selected_account', function(e) {
        ...
        global_default_main_group_number = main_group_number;
    ...
    function addNewRow() {
        ...
        html += '<input type="number" step="any" name="debit_amounts[]" class="form-control fw-bold d-none spinner_hidden text-end" id="debit_amount" value="0.00">';
        html += '</td>';

        html += '<td>';
        html += '<p class="m-0 p-0 fw-bold" id="show_credit_amount"></p>';
        html += '<input type="number" step="any" name="credit_amounts[]" class="form-control fw-bold d-none spinner_hidden text-end" id="credit_amount" value="0.00">';
        html += '</td>';
    ...
    function getCustomerClosingBalanceUserWise(user_id, account_id, user_name) {
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                tr.find('#main_group_number').val(global_default_main_group_number);
    ...
    function getCostCentreModal(tr) {
        ...
        if (myArray[index] != undefined) {
            ...
            myArray[index].forEach(element => {
                ...
                costCentreTr += '<input type="text" data-only_type="'+ (main_group_number == 3 ? 'expense' : 'income') +'" class="form-control fw-bold" id="search_cost_centre" value="'+element.cost_centre_name+'" autocomplete="off">';
                costCentreTr += '<input type="hidden" class="voidable" id="cost_centre_name">';
                costCentreTr += '<input type="hidden" class="voidable" id="default_cost_centre_name" name="default_cost_centre_names['+index+'][]" value="'+ element.cost_centre_name +'">';
                costCentreTr += '<input type="hidden" name="cost_centre_ids['+index+'][]" class="voidable" id="cost_centre_id" value="'+element.cost_centre_id+'">';
                var generate_cost_centre_unique_id = parseInt(Date.now() + Math.random() + __index);
                costCentreTr += '<input type="hidden" class="cost_centre_row_unique_id-'+ generate_cost_centre_unique_id +'" id="cost_centre_row_unique_id" value="'+ generate_cost_centre_unique_id +'">';
                costCentreTr += '</div>';
                costCentreTr += '</div>';
                costCentreTr += '</td>';

                costCentreTr += '<td>';
                costCentreTr += '<input type="number" step="any" name="cost_centre_amounts['+index+'][]" class="form-control fw-bold spinner_hidden text-end" id="cost_centre_amount" value="'+ element.cost_centre_amount +'">';
                costCentreTr += '</td>';
                ...
                if (__index == 0) {
                    ...
                    costCentreTr += '<a href="#" onclick="return false;" tabindex="-1" class="d-inline"><i class="fas fa-trash-alt text-secondary mt-1"></i></a>';
                }else{

                    costCentreTr += '<a href="#" id="remove_cost_centre_btn" class="table_tr_remove_btn d-inline"><i class="fas fa-trash-alt text-danger mt-1"></i></a>';
                }

                costCentreTr += '</div>';

                costCentreTr += '<div class="col-md-6">';
                costCentreTr += '<a href="#" id="add_cost_centre_btn" class="table_tr_add_btn ms-1 d-inline"><i class="fa-solid fa-plus text-success mt-1"></i></a>';
                costCentreTr += '</div>';
                costCentreTr += '</div>';
                costCentreTr += '</td>';
                costCentreTr += '</tr>';
            ...
            costCentreTr += '<div class="row py-1">';
            costCentreTr += '<div class="col-12">';
            costCentreTr += '<input type="text" data-only_type="'+ (main_group_number == 3 ? 'expense' : 'income') +'" class="form-control fw-bold" id="search_cost_centre" autocomplete="off">';
            costCentreTr += '<input type="hidden" class="voidable" id="cost_centre_name">';
            costCentreTr += '<input type="hidden" class="voidable" id="default_cost_centre_name" name="default_cost_centre_names[]">';
            ...
            costCentreTr += '<td>';
            costCentreTr += '<input type="number" step="any" name="cost_centre_amounts[]" class="form-control fw-bold spinner_hidden text-end" id="cost_centre_amount">';
            costCentreTr += '</td>';
...
<script>
    ...
    $('#edit_journal_form').on('submit', function(e){
        e.preventDefault();
