...
@endpush
@section('title', 'Edit Contra (Double Entry) - ')
@section('content')
        ...
        <div class="container-fluid p-0">
            <div class="row g-0">
                <div class="form_element mt-0 border-0">
                    <div class="sec-name">
                        <h6>@lang('menu.edit_contra_double_entry') | <strong>@lang('menu.voucher_no') : {{ $contra->voucher_no }}</strong></h6>
                        <div>
            ...
            <form id="edit_contra_multiple_mode_form" action="{{ route('vouchers.contras.update', $contra->id) }}" method="POST">
                @csrf
                                                ...
                                                <div class="table-responsive">
                                                    <table class="display data__table table-striped">
                                                        ...
                                                        <tbody id="multiple_mode_account_list">
                                                            @foreach ($contra->descriptions as $description)
                                                                <tr data-active_disabled="1">
                                                                    <td>
                                                                        <div class="row py-1">
                                                                            <div class="col-2">
                                                                                <input {{ $loop->index == 0 ? 'readonly' : '' }} type="text" name="amount_types[]" id="amount_type" maxlength="2" class="form-control fw-bold" value="{{ $description->amount_type == 'dr' ? 'Dr' : 'Cr' }}" {{ $loop->index == 0 ? 'tabindex="-1"' : '' }}>
                                                                            </div>

                                                                            <div class="col-6">
                                                                                @php
                                                                                    $accuntNumber = $description?->account?->account_number ? ' - A/c No.: ' . $description?->account?->account_number : '';
                                                                                @endphp
                                                                                <input type="text" data-only_type="bank_or_cash_accounts" class="form-control fw-bold" id="search_account" value="{{ $description?->account?->name . $accuntNumber }}" autocomplete="off">
                                                                                <input type="hidden" id="account_name" value="{{ $description?->account?->name . $accuntNumber }}">
                                                                                <input type="hidden" id="default_account_name" value="{{ $description?->account?->name }}">
                                                                                <input type="hidden" name="account_ids[]" id="account_id" value="{{ $description->account_id }}">
                                                                                <input type="hidden" name="payment_method_ids[]" id="payment_method_id" value="{{ $description->payment_method_id }}">
                                                                                <input type="hidden" name="transaction_nos[]" id="transaction_no" value="{{ $description->transaction_no }}">
                                                                                <input type="hidden" name="cheque_nos[]" id="cheque_no" value="{{ $description->cheque_no }}">
                                                                                <input type="hidden" name="cheque_serial_nos[]" id="cheque_serial_no" value="{{ $description->cheque_serial_no }}">
                                                                                <input type="hidden" name="cheque_issue_dates[]" id="cheque_issue_date" value="{{ $description->cheque_issue_date }}">
                                                                                <input type="hidden" name="contra_description_ids[]" id="contra_description_id" value="{{ $description->id }}">

                                                                                {{-- <p class="m-0 p-0" style="font-size:9px;">Type : <span id="sapn_payment_method">Cheque</span></p>
                                                                            <p class="m-0 p-0" style="font-size:9px;">ChequeNo : <span id="sapn_cheque_no">01144442525</span></p>
                                                                            <p class="m-0 p-0" style="font-size:9px;" id="sapn_cheque_serial_no">ChequeSerialNo : <span>15-5555</span></p>
                                                                            <p class="m-0 p-0" style="font-size:9px;" id="sapn_cheque_issue_date">ChequeIssueDate : <span>15-02-2023</span></p> --}}
                                                                            ...
                                                                            <div class="col-4">
                                                                                @php
                                                                                    $amounts = $accountUtil->accountClosingBalance($description->account_id);
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <input type="number" step="any" name="debit_amounts[]" class="form-control fw-bold spinner_hidden {{ $description->amount_type == 'dr' ? '' : 'display-none' }} text-end" id="debit_amount" value="{{ $description->amount_type == 'dr' ? $description->amount : 0.0 }}">
                                                                    </td>

                                                                    <td>
                                                                        <p class="m-0 p-0 fw-bold" id="show_credit_amount">
                                                                        </p>
                                                                        <input type="number" step="any" name="credit_amounts[]" class="form-control fw-bold spinner_hidden {{ $description->amount_type == 'cr' ? '' : 'display-none' }} text-end" id="credit_amount" value="{{ $description->amount_type == 'cr' ? $description->amount : 0.0 }}">
                                                                    </td>
                                                        ...
                                                        <tfoot>
                                                            <tr>
                                                                ...
                                                                <th class="text-end" id="show_debit_total">
                                                                    {{ App\Utils\Converter::format_in_bdt($contra->debit_total) }}
                                                                </th>
                                ...
                                <div class="row align-items-center">
                                    <div class="col-md-10">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" name="remarks" class="form-control" id="remarks" value="{{ $contra->remarks }}" data-next="contra_submit_btn" placeholder="@lang('menu.remarks')">
                                        </div>
                                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        $('#edit_contra_multiple_mode_form').on('submit', function(e) {
            e.preventDefault();
