...
@endpush
@section('title', 'Edit Contra (Single Entry) - ')
@section('content')
        ...
        <div class="container-fluid p-0">
            <div class="row g-0">
                <div class="form_element mt-0 border-0">
                    <div class="sec-name">
                        <h6>@lang('menu.edit_contra_single_entry') | <strong>@lang('menu.voucher_no') : {{ $contra->voucher_no }}</strong></h6>
                        <x-back-button />
                    </div>
                </div>
            </div>

            <form id="edit_contra_single_mode_form" action="{{ route('vouchers.contras.update', $contra->id) }}" method="POST">
                @csrf
                ...
                <section class="p-15">
                                    ...
                                    <div class="row g-lg-4">
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                                <td>
                                                                    ...
                                                                    @php
                                                                        $accuntNumber = $contra?->singleModeDebitDescription?->account?->account_number ? ' - A/c No.: ' . $contra?->singleModeDebitDescription?->account?->account_number : '';
                                                                    @endphp
                                                                    <input type="text" data-only_type="bank_or_cash_accounts" data-is_single_mode_first_ac_field="1" id="search_account" class="form-control fw-bold" value="{{ $contra?->singleModeDebitDescription?->account?->name . $accuntNumber }}" autocomplete="off">
                                                                    <input type="hidden" id="account_name" value="{{ $contra?->singleModeDebitDescription?->account?->name . $accuntNumber }}">
                                                                    <input type="hidden" id="default_account_name" value="{{ $contra?->singleModeDebitDescription?->account?->name }}">
                                                                    <input type="hidden" name="account_ids[]" id="account_id" value="{{ $contra?->singleModeDebitDescription?->account_id }}" class="debit_account_id">
                                                                    <input type="hidden" name="payment_method_ids[]" id="payment_method_id" value="{{ $contra?->singleModeDebitDescription?->payment_method_id }}">
                                                                    <input type="hidden" name="transaction_nos[]" id="transaction_no" value="{{ $contra?->singleModeDebitDescription?->transaction_no }}">
                                                                    <input type="hidden" name="cheque_nos[]" id="cheque_no" value="{{ $contra?->singleModeDebitDescription?->cheque_no }}">
                                                                    <input type="hidden" name="cheque_serial_nos[]" id="cheque_serial_no" value="{{ $contra?->singleModeDebitDescription?->cheque_serial_no }}">
                                                                    <input type="hidden" name="cheque_issue_dates[]" id="cheque_issue_date" value="{{ $contra?->singleModeDebitDescription?->cheque_issue_date }}">
                                                                    <input type="hidden" name="contra_description_ids[]" id="contra_description_id" value="{{ $contra?->singleModeDebitDescription?->id }}">
                                                                    @php
                                                                        $uniqueId = uniqid();
                                                                    @endphp
                                                                    <input type="hidden" class="unique_id-{{ $uniqueId }}" id="unique_id" value="{{ $uniqueId }}">
                                                                    <input type="hidden" name="debit_amounts[]" id="debit_amount" value="{{ $contra->debit_total }}">
                                                                    <input type="hidden" name="credit_amounts[]" id="credit_amount" value="0.00">

                                                                    @php
                                                                        $amounts = $accountUtil->accountClosingBalance($contra?->singleModeDebitDescription?->account_id);
                                                                        $balance = $amounts['closing_balance_string'];
                                                                    @endphp

                                                                    <p class="fw-bold text-muted curr_bl">Curr. Bal. : <span id="account_balance" class="fw-bold text-dark">{{ $balance }}</span>
                                                                    </p>
                                                                </td>
                                                            </tr>
                                                        </tbody>
                                        ...
                                        <div class="col-md-3">
                                            <div class="input-group">
                                                ...
                                                <div class="col-9">
                                                    <input type="text" name="date" id="date" class="form-control" value="{{ date(json_decode($generalSettings->business, true)['date_format'], strtotime($contra->date)) }}" data-next="is_transaction_details" placeholder="@lang('menu.date')" autocomplete="off">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <div class="col-2">
                                                    <select name="is_transaction_details" class="form-control form-select" id="is_transaction_details" data-next="search_account">
                                                        ...
                                                        <option {{ $contra->is_transaction_details == 0 ? 'selected' : '' }} value="0">@lang('menu.no')</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="table-responsive">
                                            <table class="display data__table table-striped">
                                                ...
                                                <tbody id="single_mode_account_list">
                                                    @foreach ($contra->singleModeCreditDescriptions as $creditDescription)
                                                            ...
                                                            <td>
                                                                <div class="row py-1">
                                                                    ...
                                                                    <div class="col-6">
                                                                        ...
                                                                        @endphp
                                                                        <input type="text" data-only_type="bank_or_cash_accounts" data-is_single_mode_first_ac_field="0" class="form-control fw-bold" id="search_account" value="{{ $creditDescription?->account?->name . $accuntNumber }}" autocomplete="off">
                                                                        <input type="hidden" id="account_name" value="{{ $creditDescription?->account?->name . $accuntNumber }}">
                                                                        ...
                                                                        <input type="hidden" name="contra_description_ids[]" id="contra_description_id" value="{{ $creditDescription->id }}">
                                                                        @php
                                                                            $__uniqueId = uniqid();
                                                                        @endphp
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        ...
                                                        <th class="text-end" id="show_credit_total">
                                                            {{ App\Utils\Converter::format_in_bdt($contra->credit_total) }}
                                                        </th>
                                                        <th class="text-center">...</th>
                                                    </tr>
                                                </tfoot>
                    ...
                    <div class="form_element m-0 rounded mt-1">
                        <div class="element-body p-0 pb-1 pt-1">
                            <div class="row align-items-center">
                                <div class="col-md-9">
                                    <input type="hidden" name="debit_total" id="debit_total" value="{{ $contra->debit_total }}">
                                    <input type="hidden" name="credit_total" id="credit_total" value="{{ $contra->credit_total }}">

                                    <div class="input-group">
                                        <label class="px-2"><b>@lang('menu.remarks')</b></label>
                                        <input type="text" name="remarks" class="form-control" id="remarks" value="{{ $contra->remarks }}" data-next="contra_submit_button" placeholder="@lang('menu.remarks')">
                                    </div>
                                </div>
                                    ...
                                    <div class="row justify-content-center">
                                        <div class="col-md-12 d-flex justify-content-end">
                                            <div class="loading-btn-box">
                                                ...
                                                <button type="button" id="contra_submit_button" class="btn w-auto btn-success px-5 contra_submit_button">@lang('menu.save')</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
@push('scripts')
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(accounts) {
                    ...
                    if (isSingleModeFirstAcField == 1) {
                        ...
                        li = '<li><a class="select_account text-danger" data-is_end_list="1" data-account_name="" data-default_account_name="" data-account_id="" data-sub_sub_group_number="" href="#">List-End</a></li>';
                    }
                    ...
                    $.each(accounts, function(key, account) {
                        ...
                        li += '<li>';
                        li += '<a class="select_account ' + (key == 0 && length == 1 ? 'selected_account' : '') + '" data-is_end_list="0" data-account_name="' + account.name + accuntNumber + '" data-default_account_name="' + account.name + '" data-account_id="' +
                            account.id + '" data-sub_sub_group_number="' + account.sub_sub_group_number + '" href="#"> ' + account.name + accuntNumber + groupName + '</a>';
                        li += '</li>';
                    });
        ...
        function addNewRow() {
            ...
            html += '<input type="text" data-only_type="bank_or_cash_accounts" data-is_single_mode_first_ac_field="0" class="form-control fw-bold" id="search_account" autocomplete="off">';
            ...
            html += '<input type="hidden" name="contra_description_ids[]" id="contra_description_id" value="">';
    ...
    <script>
        ...
        $('#edit_contra_single_mode_form').on('submit', function(e) {
            e.preventDefault();
