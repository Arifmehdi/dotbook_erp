...
@endpush
@section('title', 'Edit Payment (Single Entry) - ')
@section('content')
        ...
        <div class="container-fluid p-0">
            <div class="row g-0">
                <div class="form_element mt-0 border-0">
                    <div class="sec-name">
                        <h6>@lang('menu.edit_payment_single_entry') | @lang('menu.voucher_no') : {{ $payment->voucher_no }}</h6>
                        <x-back-button />
                    </div>
                </div>
            </div>

            <form id="edit_payment_single_entry_form" action="{{ route('vouchers.payments.update', $payment->id) }}"
                method="POST">
                @csrf
                                                            ...
                                                            <tbody>
                                                                <tr>
                                                                    <td>
                                                                        ...
                                                                        @php
                                                                            $accuntNumber = $payment?->singleModeCreditDescription?->account?->account_number ? ' - A/c No.: ' . $payment?->singleModeCreditDescription?->account?->account_number : '';
                                                                        @endphp
                                                                        ...
                                                                        <input type="text"
                                                                            ...
                                                                            value="{{ $payment?->singleModeCreditDescription?->account?->name . $accuntNumber }}"
                                                                            autocomplete="off">
                                                                        <input type="hidden" id="account_name"
                                                                            value="{{ $payment?->singleModeCreditDescription?->account?->name . $accuntNumber }}">
                                                                        <input type="hidden" id="default_account_name"
                                                                            value="{{ $payment?->singleModeCreditDescription?->account?->name }}">
                                                                        <input type="hidden" name="account_ids[]"
                                                                            id="account_id"
                                                                            value="{{ $payment?->singleModeCreditDescription?->account_id }}"
                                                                            class="credit_account_id">
                                                                        <input type="hidden" name="user_ids[]"
                                                                            id="user_id"
                                                                            value="{{ $payment?->singleModeCreditDescription?->user_id }}">
                                                                        <input type="hidden" name="payment_method_ids[]"
                                                                            id="payment_method_id"
                                                                            value="{{ $payment?->singleModeCreditDescription?->payment_method_id }}">
                                                                        <input type="hidden" name="transaction_nos[]"
                                                                            id="transaction_no"
                                                                            value="{{ $payment?->singleModeCreditDescription?->transaction_no }}">
                                                                        <input type="hidden" name="cheque_nos[]"
                                                                            id="cheque_no"
                                                                            value="{{ $payment?->singleModeCreditDescription?->cheque_no }}">
                                                                        <input type="hidden" name="cheque_serial_nos[]"
                                                                            id="cheque_serial_no"
                                                                            value="{{ $payment?->singleModeCreditDescription?->cheque_serial_no }}">
                                                                        <input type="hidden" name="cheque_issue_dates[]"
                                                                            id="cheque_issue_date"
                                                                            value="{{ $payment?->singleModeCreditDescription?->cheque_issue_date }}">
                                                                        <input type="hidden"
                                                                            name="payment_description_ids[]"
                                                                            id="contra_description_id"
                                                                            value="{{ $payment?->singleModeCreditDescription?->id }}">
                                                                        @php
                                                                            $uniqueId = uniqid();
                                                                        @endphp
                                                                        <input type="hidden"
                                                                        ...
                                                                        <input type="hidden" name="credit_amounts[]"
                                                                            id="credit_amount"
                                                                            value="{{ $payment->debit_total }}">
                                                                        @php
                                                                            $amounts = $accountUtil->accountClosingBalance($payment?->singleModeCreditDescription?->account_id);
                                            ...
                                            <div class="input-group">
                                                ...
                                                <div class="col-9">
                                                    <input type="text" name="date" id="date"
                                                        class="form-control"
                                                        value="{{ date(json_decode($generalSettings->business, true)['date_format'], strtotime($payment->date)) }}"
                                                        placeholder="@lang('menu.date')" placeholder="@lang('menu.date')"
                                            ...
                                            <div class="input-group">
                                                ...
                                                <div class="col-5">
                                                    <select name="is_transaction_details" class="form-control form-select"
                                                        id="is_transaction_details" data-next="search_account">
                                                        <option value="1">@lang('menu.yes')</option>
                                                        <option
                                                            {{ $payment->is_transaction_details == 0 ? 'selected' : '' }}
                                        ...
                                        <div class="payment_entry_table_area">
                                            <table class="display table table-sm">
                                                ...
                                                <tbody id="single_mode_account_list">
                                                    @foreach ($payment->singleModeDebitDescriptions as $debitDescription)
                                                                ...
                                                                <div class="row py-1">
                                                                    ...
                                                                    <div class="col-6">
                                                                        @php
                                                                            ...
                                                                            $assignedUser = $debitDescription->user ? ' - SR ' . $debitDescription->user->prefix . ' ' . $debitDescription->user->name . ' ' . $debitDescription->user->last_name : '';
                                                                        @endphp
                                                                        ...
                                                                        <input type="text" data-only_type="all"
                                                                            ...
                                                                            id="search_account"
                                                                            value="{{ $debitDescription?->account?->name . $accuntNumber . $assignedUser }}"
                                                                            autocomplete="off">
                                                                        <input type="hidden" id="account_name"
                                                                            value="{{ $debitDescription?->account?->name . $accuntNumber . $assignedUser }}">
                                                                        <input type="hidden" id="default_account_name"
                                                                        ...
                                                                        <input type="hidden" name="user_ids[]"
                                                                            id="user_id"
                                                                            value="{{ $debitDescription->user_id }}">
                                                                        <input type="hidden" name="payment_method_ids[]"
                                                                            id="payment_method_id"
                                                                        ...
                                                                        @endphp
                                                                        <input type="hidden"
                                                                            class="unique_id-{{ $__uniqueId }}"
                                                                            id="unique_id" value="{{ $__uniqueId }}">
                                                                    </div>
                            ...
                            <div class="row align-items-center">
                                <div class="col-md-9">
                                    ...
                                    <input type="hidden" name="credit_total" id="credit_total"
                                        value="{{ $payment->credit_total }}">

                                    <div class="input-group">
                                        <label class="px-2"><b>@lang('menu.remarks')</b></label>
                                        <input type="text" name="remarks" class="form-control" id="remarks"
                                            data-next="payment_submit_button" value="{{ $payment->remarks }}"
        ...
        <div class="modal-dialog double-col-modal" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title" id="exampleModalLabel">@lang('menu.assign_ac')</h6>
...
@push('scripts')
    ...
    <script>
        ...
        function addNewRow() {
            ...
            html += '<td>';
            html +=
                '<input type="number" step="any" name="debit_amounts[]" class="form-control fw-bold spinner_hidden text-end" id="debit_amount" value="0.00">';
    ...
    <script>
        ...
        $('#edit_payment_single_entry_form').on('submit', function(e) {
            e.preventDefault();
