...
@endpush
@section('title', 'Add Receipt (Double Entry) - ')
@section('content')
        ...
        <div class="container-fluid p-0">
            <div class="row g-0">
                <div class="form_element mt-0 border-0">
                    <div class="sec-name">
                        <h6>@lang('menu.add_receipt_double_entry')</h6>
                        <x-back-button />
                    </div>
                </div>
            </div>

            <form id="add_receipt_double_entry_form" action="{{ route('vouchers.receipts.store') }}" method="POST">
                @csrf
                                    ...
                                    <div class="row g-lg-4">
                                        <div class="col-md-3">
                                            <div class="input-group">
                                                ...
                                                <div class="col-9">
                                                    <input required type="text" name="date" id="date" class="form-control" value="{{ date(json_decode($generalSettings->business, true)['date_format']) }}" data-next="is_transaction_details" placeholder="@lang('menu.date')" autocomplete="off">
...
@push('scripts')
            ...
            $.ajax({
                ...
                success: function(accounts) {
                    ...
                    $.each(accounts, function(key, account) {
                        ...
                        li += '<li>';
                        li += '<a class="select_account ' + (key == 0 && length == 1 ? 'selected_account' : '') + '" data-is_customer="' + (account.customer_id != null ? 1 : 0) + '" data-account_name="' + account.name + accuntNumber + '" data-default_account_name="' + account.name + '" data-account_id="' + account.id + '" data-sub_sub_group_number="' + account.sub_sub_group_number + '" href="#"> ' + account.name + accuntNumber + groupName + '</a>';
                        li += '</li>';
                    });
    ...
    <script>
        ...
        $('#add_receipt_double_entry_form').on('submit', function(e) {
            e.preventDefault();
        ...
        function afterCreateSale() {
            ...
            $('.voidable').html('');
            $('.removable').remove();
            $('#add_receipt_double_entry_form')[0].reset();
