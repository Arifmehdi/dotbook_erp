...
@endpush
@section('title', 'Edit Receipt (Double Entry) - ')
@section('content')
        ...
        <div class="container-fluid p-0">
            <div class="row g-0">
                <div class="form_element mt-0 border-0">
                    <div class="sec-name">
                        <h6>@lang('menu.edit_receipt_double_entry') | {{ __('Voucher No') }} : {{ $receipt->voucher_no }}</h6>
                        <x-back-button />
                    </div>
                </div>
            </div>

            <form id="edit_receipt_double_entry_form" action="{{ route('vouchers.receipts.update', $receipt->id) }}" method="POST">
                @csrf
                                    ...
                                    <div class="row g-lg-4">
                                        <div class="col-md-3">
                                            <div class="input-group">
                                                ...
                                                <div class="col-9">
                                                    <input type="text" name="date" id="date" class="form-control" value="{{ date(json_decode($generalSettings->business, true)['date_format'], strtotime($receipt->date)) }}" data-next="is_transaction_details" placeholder="@lang('menu.date')" autocomplete="off">
                                            ...
                                            <div class="table-responsive">
                                                <table class="display data__table table-striped">
                                                    ...
                                                    <tbody id="multiple_mode_account_list">
                                                        @foreach ($receipt->descriptions as $receiptDescription)
                                                            <tr data-active_disabled="1">
                                                                <td>
                                                                    <div class="row py-1">
                                                                        <div class="col-2">
                                                                            <input {{ $loop->index == 0 ? 'readonly' : '' }} type="text" name="amount_types[]" id="amount_type" maxlength="2" class="form-control fw-bold" value="{{ $receiptDescription->amount_type == 'dr' ? 'Dr' : 'Cr' }}" {{ $loop->index == 0 ? 'tabindex="-1"' : '' }}>
                                                                        </div>

                                                                        <div class="col-6">
                                                                            @php
                                                                                $accuntNumber = $receiptDescription?->account?->account_number ? ' - A/c No.: ' . $receiptDescription?->account?->account_number : '';
                                                                                $assignedUser = $receiptDescription->user ? ' - SR ' . $receiptDescription->user->prefix . ' ' . $receiptDescription->user->name . ' ' . $receiptDescription->user->last_name : '';
                                                                            @endphp
                                                                            <input type="text" data-only_type="all" class="form-control fw-bold" id="search_account" value="{{ $receiptDescription?->account?->name . $accuntNumber . $assignedUser }}" autocomplete="off">
                                                                            <input type="hidden" id="account_name" value="{{ $receiptDescription?->account?->name . $accuntNumber . $assignedUser }}">
                                                                            <input type="hidden" id="default_account_name" value="{{ $receiptDescription?->account?->name }}">
                                                                            <input type="hidden" name="account_ids[]" id="account_id" value="{{ $receiptDescription->account_id }}">
                                                                            <input type="hidden" name="user_ids[]" id="user_id" value="{{ $receiptDescription->user_id }}">
                                                                            <input type="hidden" name="payment_method_ids[]" id="payment_method_id" value="{{ $receiptDescription->payment_method_id }}">
                                                                            <input type="hidden" name="transaction_nos[]" id="transaction_no" value="{{ $receiptDescription->transaction_no }}">
                                                                            <input type="hidden" name="cheque_nos[]" id="cheque_no" value="{{ $receiptDescription->cheque_no }}">
                                                                            <input type="hidden" name="cheque_serial_nos[]" id="cheque_serial_no" value="{{ $receiptDescription->cheque_serial_no }}">
                                                                            <input type="hidden" name="cheque_issue_dates[]" id="cheque_issue_date" value="{{ $receiptDescription->cheque_issue_date }}">
                                                                            <input type="hidden" name="remarkable_notes[]" id="remarkable_note" value="{{ $receiptDescription->remarkable_note }}">
                                                                            <input type="hidden" name="payment_description_ids[]" id="payment_description_id" value="{{ $receiptDescription->id }}">
                                                                            @php
                                                                                $uniqueId = uniqid();
                                                                            @endphp
                                                                        ...
                                                                        <div class="col-4">
                                                                            @php
                                                                                $amounts = $accountUtil->accountClosingBalance($receiptDescription->account_id, $receiptDescription->user_id);
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <input type="number" step="any" name="debit_amounts[]" class="form-control fw-bold spinner_hidden {{ $receiptDescription->amount_type == 'dr' ? '' : 'display-none' }} text-end" id="debit_amount" value="{{ $receiptDescription->amount_type == 'dr' ? $receiptDescription->amount : 0.0 }}">
                                                                </td>

                                                                <td>
                                                                    <p class="m-0 p-0 fw-bold" id="show_credit_amount">
                                                                    </p>
                                                                    <input type="number" step="any" name="credit_amounts[]" class="form-control fw-bold spinner_hidden {{ $receiptDescription->amount_type == 'cr' ? '' : 'display-none' }} text-end" id="credit_amount" value="{{ $receiptDescription->amount_type == 'cr' ? $receiptDescription->amount : 0.0 }}">
                                                                </td>
                                                    ...
                                                    <tfoot>
                                                        <tr>
                                                            ...
                                                            <th class="text-end" id="show_debit_total">
                                                                {{ App\Utils\Converter::format_in_bdt($receipt->debit_total) }}
                                                            </th>
...
@push('scripts')
    ...
    <script>
        ...
        $('#edit_receipt_double_entry_form').on('submit', function(e) {
            e.preventDefault();
