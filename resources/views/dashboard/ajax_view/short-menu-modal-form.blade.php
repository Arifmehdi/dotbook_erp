            ...
            <ul class="list-unstyled">
                @foreach ($shortMenus as $sm)
                    @if (Route::has($sm->url))
                        ...
                        @endphp
                        <li>
                            <p><input name="menu_ids[]" {{ $smu ? 'CHECKED' : '' }} type="checkbox"
                                    value="{{ $sm->id }}" id="check_menu">
                                <i class="{{ $sm->icon }} text-primary s-menu-icon ms-1"></i> <span
                                    class="s-menu-text">{{ $sm->name }}</span>
                            </p>
                        </li>
                    @endif
                @endforeach
