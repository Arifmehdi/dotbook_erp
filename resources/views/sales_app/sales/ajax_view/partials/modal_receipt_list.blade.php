...
<div class="modal_payments_or_receipts_list_table">
    <div class="table-responsive">
        <table class="table modal-table table-striped table-sm">
                ...
                @if (count($sale->references) > 0)
                   ...
                   @foreach ($sale->references as $reference)
                        ...
                        <tr>
                            ...
                            <td><b>{{ date(json_decode($generalSettings->business, true)['date_format'], strtotime($date)) }}</b></td>
                            ...
                            <td class="action_hideable">
                                @if ($reference->paymentDescription->payment->payment_type == 1)
                                    ...
                                    <a href="{{ route('vouchers.receipts.show', [$reference->paymentDescription->payment->id]) }}" id="extra_details_btn" class="btn btn-sm btn-primary">@lang('menu.details')</a>
                                @else
                                    <a href="{{ route('vouchers.payments.show', [$reference->paymentDescription->payment->id]) }}" id="extra_details_btn" class="btn btn-sm btn-primary">@lang('menu.details')</a>
                                @endif
                            </td>
                        </tr>
                   @endforeach
            ...
            <tfoot>
                <tr>
                    <th colspan="5" class="text-end">@lang('menu.total_received_against_reference') : </th>
