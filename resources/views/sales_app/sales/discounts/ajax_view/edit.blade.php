<form id="edit_discount_form" action="{{ route('sales.discounts.update', $discount->id) }}" method="POST">
    ...
    <div class="form-group row">
        <div class="col-md-12">
            ...
            <input type="text" name="name" class="form-control add_input" data-name="Offer name" id="name"
                required placeholder="Discount name" value="{{ $discount->name }}" autocomplete="off" />
    ...
    <div class="form-group row mt-1">
        <div class="col-md-12">
            <label><strong>@lang('menu.priority') <i data-bs-toggle="tooltip" data-bs-placement="right"
                        title="Leave empty to auto generate." class="fas fa-info-circle tp"></i> :</strong> <span
                    class="text-danger">*</span> </label>
            <input type="text" name="priority" step="any" class="form-control add_input" data-name="Priority"
                id="priority" placeholder="@lang('menu.priority')" value="{{ $discount->priority }}" autocomplete="off"
                required />
    ...
    <div class="form-group row mt-1">
        <div class="col-md-6">
            ...
            <input type="text" name="start_at" id="e_start_at" class="form-control add_input"
                value="{{ date(json_decode($generalSettings->business, true)['date_format'], strtotime($discount->start_at)) }}">
            <span class="error error_start_at"></span>
        </div>

        <div class="col-md-6">
            <label><strong>@lang('menu.end_at') </strong></label>
            <input type="text" name="end_at" id="e_end_at" class="form-control add_input"
                value="{{ date(json_decode($generalSettings->business, true)['date_format'], strtotime($discount->end_at)) }}">
    ...
    <div class="form-group row mt-1">
        <div class="col-md-12">
            ...
            <select name="product_ids[]" class="form-control select2 form-select" multiple="multiple"
                id="e_product_ids">
                @foreach ($products as $product)
                    <option
                        @foreach ($discountProducts as $discountProduct)
                            {{ $product->id == $discountProduct->product_id ? 'SELECTED' : '' }} @endforeach
        ...
        <div class="col-md-6">
            ...
            <select name="category_id" id="category_id" class="form-control add_input form-select">
                ...
                @foreach ($categories as $category)
                    <option {{ $discount->category_id == $category->id ? 'SELECTED' : '' }}
                        value="{{ $category->id }}">
                        {{ $category->name }}
                    </option>
                @endforeach
    ...
    <div class="form-group row mt-1">
        <div class="col-md-6">
            ...
            <select name="discount_type" id="discount_type" class="form-control add_input form-select">
                <option {{ $discount->discount_type == 1 ? 'SELECTED' : '' }} value="1">@lang('menu.fixed')(0.00)
                </option>
                <option {{ $discount->discount_type == 2 ? 'SELECTED' : '' }} value="1">@lang('menu.percentage')(%)
                </option>
            </select>
        ...
        <div class="col-md-6">
            ...
            <input type="number" name="discount_amount" id="discount_amount" class="form-control add_input"
                value="{{ $discount->discount_amount }}" autocomplete="off">
        ...
        <div class="col-md-6">
            ...
            <select name="price_group_id" id="price_group_id" class="form-control form-select">
                ...
                @foreach ($price_groups as $price_group)
                    <option {{ $discount->price_group_id == $price_group->id ? 'SELECTED' : '' }}
                ...
                <div class="col-12">
                    <div class="row">
                        <p class="checkbox_input_wrap">
                            <input {{ $discount->apply_in_customer_group == 1 ? 'CHECKED' : '' }} type="checkbox"
                                name="apply_in_customer_group" id="apply_in_customer_group"> &nbsp;
        ...
        <div class="col-md-6">
            <div class="input-group mt-1">
                <div class="col-12">
                    <div class="row">
                        <p class="checkbox_input_wrap">
                            <input {{ $discount->is_active == 1 ? 'CHECKED' : '' }} type="checkbox" name="is_active"
                                id="is_active"> &nbsp; @lang('menu.is_active')
                        </p>
                    </div>
                </div>
            </div>
