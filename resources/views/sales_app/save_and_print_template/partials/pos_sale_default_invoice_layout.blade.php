                            ...
                            <tr>
                                ...
                                <td class="text-start">
                                    ...
                                    @if ($saleProduct->variant)
                                        -{{ $saleProduct->variant->variant_name }}{!! $saleProduct->ex_quantity != 0 ? '(<b>EX:</b>'.$saleProduct->ex_quantity.')' : '' !!}
                                    @endif
                                ...
                                @if ($defaultLayout->product_w_type || $defaultLayout->product_w_duration || $defaultLayout->product_w_discription)
                                    <td class="text-start">
                                        @if ($saleProduct->product->warranty)
                                            ...
                                            {!! $defaultLayout->product_w_discription ? '<br><small class="text-muted">' . $saleProduct->product->warranty->description . '</small>' : '' !!}
                                        @else
                                            <b>@lang('menu.no')</b>
                                        @endif
                                    </td>
                                ...
                                <td class="text-start">{{ App\Utils\Converter::format_in_bdt($saleProduct->unit_price_inc_tax) }} </td>
                ...
                <div class="col-md-6">
                    <table class="table modal-table table-sm">
                        <tbody>
                            ...
                            <tr>
                                <td class="text-endx"><strong> <strong> @lang('menu.order_tax') : {{ json_decode($generalSettings->business, true)['currency'] }}</strong></td>
                            ...
                            @if ($previous_due != 0)
                                <tr>
                                    <td class="text-endx"><strong> @lang('menu.previous_due') : {{ json_decode($generalSettings->business, true)['currency'] }}</strong></td>
                            ...
                            @if ($change_amount > 0)
                                <tr>
                                    <td class="text-endx"><strong>@lang('menu.change_amount') : {{ json_decode($generalSettings->business, true)['currency'] }}</strong></td>
                                    <td class="total_paid text-end">
                            ...
                            @foreach ($sale->saleProducts as $saleProduct)
                                <tr>
                                    ...
                                    <th class="text-startx">
                                        {{ $loop->index + 1 }}. {{ $saleProduct->product->name.$variant }}{!! $saleProduct->ex_quantity != 0 ? '(<strong>EX:</strong>'.$saleProduct->ex_quantity.')' : '' !!}
                                    </th>
                ...
                <div class="amount_area">
                    <table class="w-100">
                        <thead>
                            ...
                            @if ($previous_due > 0)
                                <tr>
                                    <th class="text-endx">@lang('menu.previous_due') : {{ json_decode($generalSettings->business, true)['currency'] }} </th>
                                    <th class="text-endx">
                                        <span>
                                            {{ App\Utils\Converter::format_in_bdt($previous_due) }}
                                        </span>
                            ...
                            <tr>
                                <th class="text-endx"> @lang('menu.paid') : {{ json_decode($generalSettings->business, true)['currency'] }}</th>
                                <th class="text-endx">
                                    <span>
                                        {{ App\Utils\Converter::format_in_bdt($paying_amount) }}
                                    </span>
                            ...
                            <tr>
                                <th class="text-endx"> @lang('menu.due') : {{ json_decode($generalSettings->business, true)['currency'] }}</th>
                                <th class="text-endx">
                                    <span>
