...
@endphp
    @foreach ($products as $product)
        ...
        @else
            ...
            @endphp
            <div class="col-lg-4 col-sm-3 col-4">
                <a href="#" tabindex="-1" title="{{ $product->name }}" href="#" class="select_single_product" data-p_id="{{ $product->id }}" data-p_name="{{ $product->name }}" data-p_tax_ac_id="{{ $product->tax_ac_id }}" data-p_code="{{ $product->product_code }}" data-unit="{{ $product->unit_name }}" data-p_tax_percent="{{ $tax_percent }}" data-tax_type="{{ $product->tax_type }}" data-p_tax_amount="{{ bcadd((float) $tax_amount, 0, 2) }}" data-description="{{ $product->is_show_emi_on_pos }}" data-p_price_exc_tax="{{ $product->product_price }}" data-p_cost_inc_tax="{{ $product->product_cost_with_tax }}" onclick="singleProduct(this); return false;">
                    <div class="product">
                        <div class="product-img">
                            <img loading="lazy" src="{{ asset('uploads/product/thumbnail/'.$product->thumbnail_photo) }}">
                        </div>
