                        ...
                        <div class="col-md-4 col-sm-4 col-lg-4">
                            <div class="middle_header_text text-center">
                                ...
                                <h6>
                                    ...
                                    @if ($sale->due <= 0)
                                    @lang('menu.paid')
                                    @elseif ($sale->due > 0 && $sale->due < $payable) @lang('menu.partial')
                    ...
                    <tbody class="sale_print_product_list">
                        @foreach ($sale->saleProducts as $saleProduct)
                            <tr>
                                ...
                                @if ($defaultLayout->product_w_type || $defaultLayout->product_w_duration || $defaultLayout->product_w_discription)
                                    <td class="text-start">
                                        @if ($saleProduct->product->warranty)
                                            ...
                                            {{ $saleProduct->product->warranty->type == 1 ? 'Warrantiy' : 'Guaranty' }}
                                            {!! $defaultLayout->product_w_discription ? '<br><small class="text-muted">' . ($saleProduct->description == 'null' ? '' : $saleProduct->description) . '</small>' : '' !!}
                                        @else
                                            <b>@lang('menu.no')</b>
                                        @endif
                                    </td>
                                ...
                                <td class="text-start">{{ $saleProduct->unit_price_inc_tax }} </td>
                                @if ($defaultLayout->product_discount)
                                    <td class="text-start">
                                ...
                                @if ($defaultLayout->product_tax)
                                    <td class="text-start">{{ $saleProduct->unit_tax_percent }}%</td>
                                @endif

                                <td class="text-start">{{ $saleProduct->subtotal }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
