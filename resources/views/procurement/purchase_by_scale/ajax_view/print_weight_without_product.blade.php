    ...
    <div class="purchase_print_template">
        <div class="details_area">
            ...
            <div class="purchase_and_deal_info mt-1">
                <div class="row">
                    <div class="col-12 text-center">
                        <h4 style="text-transform: uppercase; border-bottom: 1px solid black; padding-bottom:5px;"><strong>@lang('menu.weight_details')</strong></h4>
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-4">
                        <ul class="list-unstyled">
                            ...
                            <li style="font-size:12px!important;"><strong>@lang('menu.scale_operator') : </strong> {{ $purchaseByScale->createdBy ? $purchaseByScale->createdBy->prefix.' '.$purchaseByScale->createdBy->name.' '.$purchaseByScale->createdBy->last_name : 'N/A' }}</li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <table class="table report-table table-sm table-bordered print_table">
                    ...
                    @foreach ($purchaseByScale->weights as $weight)
                        @if ($loop->index == 0)
                            ...
                            <tr>
                                <td>
                                    <strong>{{ $serialArr[$loop->index] }} Weight (Gross Weight) </strong>
                                </td>
                                <td><strong>= {{ App\Utils\Converter::format_in_bdt($weight->scale_weight) }}</strong> Kg </td>
                                <td>---</td>
                                <td><strong>Date & Time :</strong> {{ date(json_decode($generalSettings->business, true)['date_format'].' h:i:s A', strtotime($weight->created_at)) }}</td>
                            </tr>
                        ...
                        @else
                            ...
                            <tr>
                                <td><strong>{{ $serialArr[$loop->index] }} Weight {{ ($loop->index) == $lastWeight ? '(Last Weight)' : ''  }} </strong></td>
                                <td><strong>= {{ App\Utils\Converter::format_in_bdt($weight->scale_weight) }} </strong> Kg </td>
                                <td>
                                    @php
            ...
            <div class="row">
                <div class="col-12">
                    <p style="font-size:12px!important;"><strong>@lang('menu.net_weight') = </strong> {{ App\Utils\Converter::format_in_bdt($purchaseByScale->net_weight) }} <strong>Kg</strong></p>
                </div>
            </div>
