...
<div class="row mt-2">
    <div class="col-12 text-center">
        <h6 style="text-transform:uppercase;"><strong>@lang('menu.purchase_returned_items_report') </strong></h6>
    </div>
</div>
        ...
        <table class="table report-table table-sm table-bordered print_table">
            <thead>
                <tr>
                    ...
                    <th class="text-end">@lang('menu.unit_cost_exc_tax')</th>
            ...
            <tbody class="sale_print_product_list">
                ...
                @foreach ($purchaseReturnProducts as $purchaseReturnProduct)
                    @php
                        $date = date($__date_format, strtotime($purchaseReturnProduct->report_date));
                    @endphp
                    ...
                    <tr>
                        <td class="text-start">
                            @php
                                $variant = $purchaseReturnProduct->variant_name ? ' - ' . $purchaseReturnProduct->variant_name : '';

                            @endphp
                            {{ $purchaseReturnProduct->name . $variant }}
                        </td>

                        <td class="text-start">{{ $purchaseReturnProduct->supplier_name }}</td>
                        <td class="text-start fw-bold">{{ $purchaseReturnProduct->voucher_no }}</td>
                        @php
                            $baseUnitMultiplier = $purchaseReturnProduct->base_unit_multiplier ? $purchaseReturnProduct->base_unit_multiplier : 1;
                            $returnedQty = $purchaseReturnProduct->return_qty / $baseUnitMultiplier;
                            $totalQty += $returnedQty;
                            $totalSubTotal += $purchaseReturnProduct->return_subtotal;
                        @endphp
                        <td class="text-end fw-bold">{!! \App\Utils\Converter::format_in_bdt($returnedQty) . '/' . $purchaseReturnProduct->unit_code !!}</td>

                        <td class="text-end fw-bold">
                            {{ \App\Utils\Converter::format_in_bdt($purchaseReturnProduct->unit_cost_exc_tax * $baseUnitMultiplier) }}
                        </td>

                        <td class="text-end fw-bold">{{ \App\Utils\Converter::format_in_bdt($purchaseReturnProduct->unit_discount_amount * $baseUnitMultiplier) }}</td>

                        <td class="text-end fw-bold">
                            {{ '(' . \App\Utils\Converter::format_in_bdt($purchaseReturnProduct->unit_tax_percent) . '%)=' . \App\Utils\Converter::format_in_bdt($purchaseReturnProduct->unit_tax_amount * $baseUnitMultiplier) }}
                        </td>

                        <td class="text-end fw-bold">{{ \App\Utils\Converter::format_in_bdt($purchaseReturnProduct->unit_cost_inc_tax * $baseUnitMultiplier) }}</td>
                        <td class="text-end fw-bold">{{ \App\Utils\Converter::format_in_bdt($purchaseReturnProduct->return_subtotal) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="col-6">
        <table class="table report-table table-sm table-bordered print_table">
            <thead>
                ...
                <tr>
                    <th class="text-end">@lang('menu.total_returned_amount') : {{ json_decode($generalSettings->business, true)['currency'] }}</th>
