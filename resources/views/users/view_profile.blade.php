                            ...
                            <div class="card-body p-2">
                                <div class="profile-sidebar">
                                    ...
                                    <ul class="profile-short-info">
                                        <li>{{ __('Employee ID') }}<span>{{ auth()->user()->employee_id }}</span></li>
                                        {{-- <li>@lang('menu.role')<span>{{ $user?->roles()?->first()?->name }}</span></li> --}}
                                        <li>{{ __('Designation') }}<span>{{ auth()->user()->designation }} Faculty</span></li>
                                        <li>{{ __('Departments') }}<span>{{ auth()->user()->departments }} Academic</span></li>
                                        <li>{{ __('Employee Type') }}<span>{{ auth()->user()->permanent_village }} Permanent</span></li>
                                        <li>{{ __('Shift') }}<span>{{ auth()->user()->shift_id }} Morning</span></li>
                                        <li>{{ __('Joining Date') }}<span>{{ auth()->user()->joining_date }} 03/10/2022</span></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
