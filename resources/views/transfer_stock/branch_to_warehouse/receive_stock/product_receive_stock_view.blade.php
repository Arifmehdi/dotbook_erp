...
@section('content')
    <div class="body-wraper">
        <div class="container-fluid p-0">
            ...
            <form id="receive_stock_form"
                action="{{ route('transfer.stocks.to.warehouse.receive.stock.process.save', $sendStockId) }}" method="POST">
                @csrf
                <section class="p-15">
                        ...
                        <div class="row">
                            <div class="form_element rounded mt-0 mb-1">
                                ...
                                <div class="element-body">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <p class="m-0"><strong>@lang('menu.reference_id'): </strong> <span
                                                    class="transfer_invoice_id"></span> </p>
                                            <p class="m-0"><strong>@lang('menu.date'): </strong> <span
                                                    class="transfer_date"></span></p>
                                        </div>
                                        <div class="col-md-6">
                                            <p class="m-0"><strong>@lang('menu.warehouse') (From) : </strong> <span
                                                    class="warehouse"></span> </p>
                                            <p class="m-0"><strong>@lang('menu.business_location') : </strong> <span
                                                    class="branch"></span></p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="input-group">
                                        <label class="col-4">@lang('menu.receiver') @lang('menu.note')</label>
                                        <div class="col-8">
                                            <input type="text" name="receiver_note" id="receiver_note"
                                                class="form-control" placeholder="@lang('menu.receiver') @lang('menu.note')">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-3">
                                    <div class="input-group">
                                        ...
                                        <div class="col-8">
                                            <input readonly type="number" step="any" name="total_received_quantity"
                                                id="total_received_quantity" class="form-control">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-12 d-flex justify-content-end">
                                <div class="loading-btn-box">
                                    ...
                                    <button class="btn btn-success">@lang('menu.save')</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        function getReceiveableStock() {
            $.ajax({
                ...
                success: function(sendStock) {
                    ...
                    $('.warehouse').html(sendStock.warehouse.warehouse_name + '/' + sendStock.warehouse
                        .warehouse_code);
                    $('.branch').html(sendStock.branch ? sendStock.branch.name + '/' + sendStock.branch
                        .branch_code : headBranch);
                    ...
                    $.each(sendStock.transfer_products, function(key, sendProduct) {
                        ...
                        var variant = sendProduct.product_variant_id != null ? ' -' + sendProduct
                            .variant.variant_name + '- ' : '';
                        tr += '<span class="product_variant">' + variant + '</span>';
                        var code = sendProduct.product_variant_id != null ? sendProduct.variant
                            .variant_code : sendProduct.product.product_code;
                        tr += '<span class="product_code">' + '(' + code + ')' + '</span>';
                        tr += '</a>';
                        tr += '<input value="' + sendProduct.product_id +
                            '" type="hidden" class="productId-' + sendProduct.product_id +
                            '" id="product_id" name="product_ids[]">';

                        if (sendProduct.product_variant_id != null) {
                            tr += '<input value="' + sendProduct.product_variant_id +
                                '" type="hidden" class="variantId-' + sendProduct.product_variant_id +
                        ...
                        tr +=
                            '<input type="hidden" name="previous_received_quantities[]" id="previous_received_quantity" value="' +
                            sendProduct.received_qty + '">';
                        ...
                        var pending_qty = parseFloat(sendProduct.quantity) - parseFloat(sendProduct
                            .received_qty);
                        tr += '<td class="text text-center">';
                        tr += '<b><span class="text-danger" id="pending_qty">' + parseFloat(pending_qty)
                            .toFixed(2) + '</span><b>';
                        tr += '</td>';

                        tr += '<td>';
                        tr += '<input value="' + sendProduct.received_qty +
                            '" required name="receive_quantities[]" type="text" class="form-control text-center form-control-sm" id="receive_quantity">';
                        tr += '</td>';
                        tr += '</tr>';
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (!$.isEmptyObject(data.errorMsg)) {
                        ...
                        window.location =
                            "{{ route('transfer.stocks.to.warehouse.receive.stock.index') }}";
                    }
                }
            });
        });
