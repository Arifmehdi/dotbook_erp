<table id="single_product_branch_stock_table" class="table modal-table table-sm">
    <thead>
        <tr class="bg-primary">
            <th class="text-white text-start">@lang('menu.item_code')(SKU)</th>
            <th class="text-white text-start">@lang('menu.item')</th>
            ...
            <th class="text-white text-start">@lang('menu.total')  @lang('menu.opening_stock')(+)</th>
    ...
    <tbody>
        @if (count($own_branch_stocks) > 0)
            @foreach ($own_branch_stocks as $row)
                @if ($row->variant_name)
                    <tr>
                        ...
                        <td class="text-start">{!! json_decode($generalSettings->business, true)['shop_name'] !!}</td>
            ...
            @endforeach
        @else
            <tr><th colspan="10" class="text-center">@lang('menu.this_item_business_location')</th></tr>
        @endif
    </tbody>
</table>
