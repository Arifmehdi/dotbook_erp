...
@section('content')
    <div class="body-wraper">
        <div class="container-fluid p-0">
            <div class="sec-name">
                <h6>@lang('menu.edit_daily_stock')</h6>
                <x-back-button />
            </div>
            <form id="edit_daily_stock_form" action="{{ route('daily.stock.update', $dailyStock->id) }}" enctype="multipart/form-data" method="POST">
                @csrf
                            ...
                            <div class="row">
                                <div class="col-xl-3 col-md-6">
                                    @if (count($warehouses) > 0)
                                        ...
                                        <div class="input-group">
                                            <label class="col-4"><span class="text-danger">*</span> <b>@lang('menu.warehouse')
                                                    :</b> </label>
                                            <div class="col-8">
                                                <select class="form-control form-select" name="warehouse_id" data-name="Warehouse" id="warehouse_id" data-next="date" autofocus>
                                                    <option value="">@lang('menu.select_warehouse')</option>
                                                    @foreach ($warehouses as $w)
                                                        <option {{ $dailyStock->warehouse_id == $w->id ? 'SELECTED' : '' }} value="{{ $w->id }}">
                                ...
                                <div class="col-xl-3 col-md-6">
                                    <div class="input-group">
                                        ...
                                        <div class="col-8">
                                            <input type="text" name="date" class="form-control" value="{{ date(json_decode($generalSettings->business, true)['date_format'], strtotime($dailyStock->date)) }}" id="date" data-next="reported_by" placeholder="dd-mm-yyyy" autofocus autocomplete="off">
                                ...
                                <div class="col-xl-3 col-md-6">
                                    <div class="input-group">
                                        ...
                                        <div class="col-8">
                                            <input type="text" name="reported_by" class="form-control" id="reported_by" value="{{ $dailyStock->reported_by }}" data-next="search_product" placeholder="@lang('menu.reported_by')" autocomplete="off">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-xl-3 col-md-6">
                                    <div class="input-group">
                                        ...
                                        <div class="col-8">
                                            <input readonly type="text" name="voucher_no" id="voucher_no" class="form-control fw-bold" placeholder="@lang('menu.voucher_no')" value="{{ $dailyStock->voucher_no }}" autocomplete="off" tabindex="-1">
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="row">
                                            <div class="col-xl-12">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-xl-2 col-md-4">
                                                        <label><b>@lang('menu.unit_tax') (@lang('menu.per')
                                                        ...
                                                        <table class="display data__table table-striped">
                                                            ...
                                                            <tbody id="stock_list">
                                                                ...
                                                                @foreach ($dailyStock->dailyStockProducts as $dsProduct)
                                                                    @php
                                                                        if (isset($dsProduct->product_id)) {
                                                                            $itemUnitsArray[$dsProduct->product_id][] = [
                                                                                'unit_id' => $dsProduct->product->unit->id,
                                                                                'unit_name' => $dsProduct->product->unit->name,
                                                                                'unit_code_name' => $dsProduct->product->unit->code_name,
                                                                        ...
                                                                        if (count($dsProduct?->product?->unit?->childUnits) > 0) {
                                                                            foreach ($dsProduct?->product?->unit?->childUnits as $unit) {
                                                                                $multiplierDetails = '(1 ' . $unit->name . ' = ' . $unit->base_unit_multiplier . '/' . $dsProduct?->product?->unit?->name . ')';

                                                                                array_push($itemUnitsArray[$dsProduct->product_id], [
                                                                                    'unit_id' => $unit->id,
                                                                                    'unit_name' => $unit->name,
                                                                        ...
                                                                        $variant = $dsProduct->variant_id ? ' -' . $dsProduct->variant->variant_name : '';
                                                                        $variantId = $dsProduct->variant_id ? $dsProduct->variant_id : 'noid';
                                                                    @endphp

                                                                    <tr class="text-start" id="select_item">
                                                                        <td>
                                                                            <span id="item_name">{{ $dsProduct->product->name . $variant }}</span>
                                                                            <input type="hidden" name="product_ids[]" value="{{ $dsProduct->product_id }}" class="productId-{{ $dsProduct->product_id }}" id="product_id" tabindex="-1">
                                                                            <input type="hidden" name="variant_ids[]" value="{{ $variantId }}" id="variant_id" tabindex="-1">
                                                                            <input type="hidden" name="daily_stock_product_ids[]" value="{{ $dsProduct->id }}" tabindex="-1">
                                                                            <input type="hidden" id="{{ $dsProduct->product_id . $variantId }}" value="{{ $dsProduct->product_id . $variantId }}" tabindex="-1">
                                                                        </td>
                                                                        ...
                                                                        <td>
                                                                            ...
                                                                            @endphp
                                                                            <span class="fw-bold" id="span_showing_quantity">{{ bcadd($dailyStockQty, 0, 2) }}</span>
                                                                            <input type="hidden" id="showing_quantity" value="{{ $dailyStockQty }}">
                                                                            <input type="hidden" name="quantities[]" value="{{ $dsProduct->quantity }}" id="quantity">
                                                                        </td>

                                                                        <td>
                                                                            <span class="fw-bold" id="span_unit">{{ $dsProduct?->dailyStockUnit?->name }}</span>
                                                                            <input type="hidden" name="unit_ids[]" step="any" id="unit_id" value="{{ $dsProduct?->unit_id }}">
                                                                        </td>

                                                                        <td>
                                                                            <span class="fw-bold" id="span_showing_unit_cost_exc_tax">{{ bcadd($dsProduct->unit_cost_exc_tax * $baseUnitMultiplier, 0, 2) }}</span>
                                                                            <input type="hidden" id="showing_unit_cost_exc_tax" value="{{ bcadd($dsProduct->unit_cost_exc_tax * $baseUnitMultiplier, 0, 2) }}">
                                                                            <input type="hidden" name="unit_costs_exc_tax[]" id="unit_cost_exc_tax" value="{{ $dsProduct->unit_cost_exc_tax }}">
                                                                        </td>

                                                                        <td>
                                                                            <span>{{ $dsProduct->tax_percent }}</span>
                                                                            <input type="hidden" name="tax_ac_ids[]" id="tax_ac_id" value="{{ $dsProduct->tax_ac_id }}">
                                                                            <input type="hidden" name="tax_percents[]" id="tax_percent" value="{{ $dsProduct->tax_percent }}">
                                                                            <input type="hidden" name="tax_types[]" id="tax_type" value="{{ $dsProduct->tax_type }}">
                                                                            <input type="hidden" name="tax_amounts[]" value="{{ $dsProduct->tax_amount }}" id="tax_amount">
                                                                            <input type="hidden" id="showing_unit_tax_amount" value="{{ bcadd($dsProduct->unit_tax_amount * $baseUnitMultiplier, 0, 2) }}">
                                                                        </td>

                                                                        <td>
                                                                            <span class="fw-bold" id="span_unit_cost_inc_tax">{{ bcadd($dsProduct->unit_cost_inc_tax * $baseUnitMultiplier, 0, 2) }}</span>
                                                                            <input type="hidden" id="showing_unit_cost_inc_tax" value="{{ bcadd($dsProduct->unit_cost_inc_tax * $baseUnitMultiplier, 0, 2) }}">
                                                                            <input type="hidden" name="unit_costs_inc_tax[]" id="unit_cost_inc_tax" value="{{ $dsProduct->unit_cost_inc_tax }}">
                                                                        </td>

                                                                        <td>
                                                                            <span id="span_subtotal" class="fw-bold">{{ $dsProduct->subtotal }}</span>
                                                                            <input type="hidden" name="subtotals[]" value="{{ $dsProduct->subtotal }}" id="subtotal">
                                                                        </td>
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="input-group mt-1">
                                                        ...
                                                        <div class="col-8">
                                                            <div class="input-group">
                                                                <input readonly name="total_item" type="number" step="any" class="form-control w-40 fw-bold" id="total_item" value="{{ $dailyStock->total_item }}" tabindex="-1">
                                                                <input readonly name="total_qty" type="number" step="any" class="form-control w-60 fw-bold" id="total_qty" value="{{ $dailyStock->total_qty }}" tabindex="-1">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-md-12">
                                                    <div class="input-group mt-1">
                                                        ...
                                                        <div class="col-8">
                                                            <input readonly name="total_stock_value" type="number" step="any" class="form-control fw-bold" id="total_stock_value" value="{{ $dailyStock->total_stock_value }}" tabindex="-1">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="input-group mt-1">
                                                        ...
                                                        <div class="col-8">
                                                            <input name="production_details" type="text" step="any" class="form-control" id="production_details" value="{{ $dailyStock->production_details }}" data-next="note" placeholder="@lang('menu.production_details')">
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-md-12">
                                                    <div class="input-group mt-1">
                                                        ...
                                                        <div class="col-8">
                                                            <input name="note" class="form-control" id="note" value="{{ $dailyStock->note }}" data-next="save" placeholder="@lang('menu.special_note')">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12 d-flex justify-content-end">
                            <div class="loading-btn-box">
                                ...
                                <button type="submit" id="save" value="2" class="btn btn-success submit_button">@lang('menu.save_change')</button>
                            </div>
                        </div>
                    </div>
                </section>
    ...
    <script>
                        ...
                        if (!$.isEmptyObject(product.product)) {
                            ...
                            if (product.variants.length == 0) {
                                ...
                                $.each(product.variants, function(key, variant) {
                                    ...
                                    li += '<li>';
                                    li += '<a href="#" class="select_variant_product" onclick="selectProduct(this); return false;" data-p_id="' +
                                        product.id + '" data-v_id="' + variant.id + '" data-p_name="' + product.name + '" data-v_name="' + variant.variant_name + '" data-p_tax_ac_id="' + (product.tax_ac_id != null ? product.tax_ac_id : '') + '" data-tax_type="' + product.tax_type + '" data-v_code="' + variant.variant_code + '" data-p_cost_exc_tax="' + variant.variant_cost + '"><img style="width:20px; height:20px;" src="' + imgUrl + '/' + product.thumbnail_photo + '"> ' + product.name + ' - ' + variant.variant_name + '</a>';
                                    li += '</li>';
                                });
        ...
        $('#add_item').on('click', function(e) {
            ...
            if (uniqueIdValue == undefined) {
                ...
                tr += '<input type="hidden" name="daily_stock_product_ids[]" tabindex="-1">';
                tr += '</td>';
        ...
        $('#edit_daily_stock_form').on('submit', function(e) {
            e.preventDefault();
