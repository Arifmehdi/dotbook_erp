...
<div class="row">
    <div class="col-md-12 text-center">
        <h6 style="margin-top: 10px; text-transform:uppercase;"><strong>@lang('menu.stock_adjusted_item_report')</strong></h6>
    </div>
        ...
        <table class="table report-table table-sm table-bordered print_table">
            ...
            <tbody class="sale_print_product_list">
                @php
                    $previousDate = '';
                @endphp
                @foreach ($stockAdjustedProducts as $stockAdjustedProduct)
                    @php
                       $date = date($__date_format, strtotime($stockAdjustedProduct->date))
                    @endphp
                    ...
                    <tr>
                        <td class="text-start">{{ $stockAdjustedProduct->variant_code ? $stockAdjustedProduct->variant_code : $stockAdjustedProduct->product_code}}</td>
                        <td class="text-start">
                            @php
                                $variant = $stockAdjustedProduct->variant_name ? ' - ' . $stockAdjustedProduct->variant_name : '';
                            @endphp
                           {{ $stockAdjustedProduct->name . $variant }}
                        </td>

                        <td class="text-start">
                            @if ($stockAdjustedProduct->w_name)

                                {!! $stockAdjustedProduct->w_name.'/'.$stockAdjustedProduct->w_code. '<strong>(WH)</strong>' !!}
                            @elseif ($stockAdjustedProduct->b_name)
                        ...
                        <td class="text-start">{{ $stockAdjustedProduct->voucher_no }}</td>
                        @php
                            $baseUnitMultiplier = $stockAdjustedProduct->base_unit_multiplier ? $stockAdjustedProduct->base_unit_multiplier : 1;
                            $adjustedQty = $stockAdjustedProduct->quantity / $baseUnitMultiplier
                        @endphp
                        <td class="text-start">{{ App\Utils\Converter::format_in_bdt($adjustedQty) . '/' . $stockAdjustedProduct->unit_code }}</td>
                        <td class="text-end">{{ App\Utils\Converter::format_in_bdt($stockAdjustedProduct->unit_cost_inc_tax * $baseUnitMultiplier) }}</td>
                        <td class="text-end">{{ App\Utils\Converter::format_in_bdt($stockAdjustedProduct->subtotal) }}</td>

                        @php
                            $totalQty += $stockAdjustedProduct->quantity;
                            $totalSubtotal += $stockAdjustedProduct->subtotal;
                        @endphp
                    </tr>
                @endforeach
            </tbody>
